...
if [ ! -f "$ENV_MARKER" ] || ! conda run -n "$ROBOCASA_ENV_NAME" python -c \
  'import robocasa, robosuite, tangying_robocasa; import trimesh, pygltflib; import PIL, scipy; from importlib.metadata import version; assert version("trimesh") == "4.12.2"; assert version("pygltflib") == "1.16.5"; assert version("Pillow") == "12.3.0"; assert version("scipy") == "1.17.1"; from tangying_robot_proto.robot.v1 import robot_pb2' \
  >/dev/null 2>&1; then
  ...
  conda run -n "$ROBOCASA_ENV_NAME" python -m pip install -e "$PROJECT_ROOT[visual]"
  touch "$ENV_MARKER"
fi
