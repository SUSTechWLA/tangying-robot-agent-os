...
cleanup() {
  for pid in "$cloud_pid" "$robot_pid"; do
...
cd "$ROOT"
go run ./cmd/cloud-control-plane --dev --listen "127.0.0.1:$cloud_port" >"$temporary/cloud.log" 2>&1 &
cloud_pid=$!
"$ROOT/.venv/bin/python" -m tangying_sim.server --listen "127.0.0.1:$robot_port" --seed "$SEED" >"$temporary/robot.log" 2>&1 &
robot_pid=$!

attempt=0
until curl -fsS "$cloud_url/healthz" >/dev/null 2>&1; do
  attempt=$((attempt + 1))
  if [ "$attempt" -ge 100 ]; then
    echo "error: cloud did not become healthy" >&2
    sed -n '1,160p' "$temporary/cloud.log" >&2
    exit 1
  fi
  sleep 0.1
done

task_json=$(curl -fsS -X POST "$cloud_url/v1/tasks" \
...
curl -fsS -X POST "$cloud_url/v1/tasks/$task_id/approve" >/dev/null

go run ./cmd/local-agent --once --dev-insecure \
  --cloud "$cloud_url" \
  --robot "127.0.0.1:$robot_port" \
  --data-dir "$temporary/local-agent" >"$temporary/local-agent.log" 2>&1

finished=$(curl -fsS "$cloud_url/v1/tasks/$task_id")
