...
v0.2.0 的真实 ROS 导航验收（历史记录，脚本仍可运行）先启动 `make navigation-start NAVIGATION_ARGS='--build --mode mapping'`，等待地图就绪，再运行 `.venv/bin/python scripts/run_navigation_acceptance.py --output 新目录`。它明确创建并批准一条本地 MuJoCo 双物体任务，要求从距操作位至少 60 厘米的位置出发，校验 18 个唯一步骤、六次物理工具各执行一次、全部历史图像哈希、抵达后新观测、真实导航位移与三帧稳定放置。`--pause-seconds 65` 在首个导航工具边界暂停后继续；只读重观测产生额外事件是正常恢复行为，不计为重复物理执行。脚本不重置现场或删除 journal；下一次演示先在任务结束且未持物时显式重启仿真。建图／已保存地图定位、暂停和故障分别留独立输出目录，结果见 v0.2.0 发布记录；当前版本的闭环契约验收见 [v0.3.0 发布记录](../releases/v0.3.0.md)。
...
GitHub runner 是两核、`MUJOCO_GL=osmesa` 的软件渲染环境，整个套件比开发机慢约 2.5 倍。因此**等待就绪的预算写在环境变量里，而不是写死在测试中**——预算约束的是"坏掉的栈"，不是"慢的栈"：

| 环境变量 | 默认值 | 作用 |
| --- | --- | --- |
| `TANGYING_E2E_STARTUP_TIMEOUT_S` | 90 | 等待栈首次上报遥测；旧值 20 秒在 CI 上会把健康的相机栈判为启动失败 |
| `TANGYING_E2E_LIFECYCLE_TIMEOUT_S` | 120 | 单次 `sim-stack.sh start/stop` 的上限；旧值 35 秒在 CI 上会被软件渲染启动超过 |
| `TANGYING_RENDER_TIMEOUT_S` | 60 | 单次 GL 渲染的上限。超过则渲染器标记为不可用并**立刻报错**，而不是无限等待 |

`pyproject.toml` 的 `timeout_method` 使用 `signal`：被挂起的测试会单独失败并打印摘要。`thread` 方法会转储全部线程并**杀掉整个进程**，于是 CI 只报告"make test 失败"，既没有失败测试名也没有摘要——排查成本极高。

CI 的 `test` 作业上限为 45 分钟（原 20 分钟），因为完整套件在软件渲染下的耗时接近甚至超过原上限。
