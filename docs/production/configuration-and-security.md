...
权威示例：`deploy/cloud/.env.example`、`deploy/local/local.env.example`、`deploy/edge-orin/edge.env.example`、`deploy/edge-orin/edge-worker.env.example`、`deploy/robot/raspberry-pi/robot-pi.env.example`。生产 `.env` 不进入 Git。
...
| `FLEET_ASSIST_MAX_OUTPUT_TOKENS` | 云端工具单次输出 token 上限，默认 4096；请求/响应分别限制 64 KiB/1 MiB |
| `LOCAL_ROBOT_ID`, `AGENT_CLOUD_ASSIST_URL`, `AGENT_CLOUD_ASSIST_DEVICE_TOKEN`, `AGENT_CLOUD_ASSIST_CA` | 单机器人身份与云端只读工具的 HTTPS 设备认证；推荐使用 `FLEET_ASSIST_DEVICE_CREDENTIALS` 对应的最小权限 token；URL 固定为 `/v1/assist` |
