...
## 9.2 自动通道（`internal/autorecovery`）

发现不再需要有人按按钮才开始被调查：

```
发现 → 出计划 → 只跑计划里的只读步骤 → 结果进任务账本
```

**线画在"目录说它 `read_only`"上**，不是画在"工具自称不改世界"上。两个问题的答案来源不同：
`MutatesWorld` 是服务对自己的声明，风险等级是目录对"运行这个动作意味着什么"的判断。
读风险等级让"能不能无人执行"只有一个答案，且在那个答案被评审的地方。
目录里 7 个会动机器人的动作全是 `bounded_write`，所以**自动通道结构上够不到它们**——
守卫和能力是同一个事实，不是一道可以被删掉检查。

**跳过也记录，跑过的不重复记。** 只记"跑了什么"的账本无法区分"系统看过并决定不碰"与"系统从没考虑过"，
而前者正是无人值守的意义；跑过的步骤由执行器记（它掌握轨迹与结果），两边都记会让一个动作在回放里出现两次。

**单工具动作不需要模型**：选择集大小为 1 时，模型不增加信息，只增加一个依赖和一个失败模式。
但**要参数的工具不调用**（编一个像样的参数发给机器人比不调用更糟），**也不调用第二次**
（定死的自动重试是本系统刻意没有的东西）。没有模型时"看"照常，"改"照停。

### 规则修正：`rule.reconcile-first` 曾经把自己锁死

契约说"结果未知时不得改动机器人"。旧实现是 **escalate 且不带步骤**——读起来正确，跑起来是死锁：
**完成对账的动作本身就是一个只读观测**，而 escalate 会丢弃所有步骤。系统能发现"世界状态不明"，
却永远无法自己离开这个状态。

现在改为提出目录里匹配的**只读**动作。模型在这一支里根本不会被咨询，所以"挡住会动机器人的提案"
是结构性的。目录里没有匹配的只读动作时，照旧 escalate。

### 自动通道的实测（真实数据，52 个任务）

| 动作 | 结果 |
| --- | --- |
| `observe.re-read` | 21 次自动执行成功 |
| `execution.read-history` | 6 次自动执行成功（接线前 8 次全部因工具缺失失败） |
| 账本 | 27 条 `ops.recovery_executed`，`operatorApproved` 为空表示不是人批的 |

**它没有消除任何一条发现，这是能力边界而不是失败**：只读对账能取证，不能判定。
`telemetry.read` 读的是机器人**当前**状态，回答不了"那一步当时完成了没有"。
要让发现真正消解，需要有人依据证据判定世界状态，而目录里没有任何动作做这件事。

## 9.1 仍然不做的事（诚实清单）

- **只读对账不能消解"结果未知"的发现**。系统能把证据摆到人面前，不能替人下结论。
