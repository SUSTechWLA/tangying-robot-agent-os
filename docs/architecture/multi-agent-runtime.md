...
本轮把 Agent 层升级为可扩展多 Agent 运行时。**当前只启用两个 Agent**：
...
| `agentruntime` | EventBus、Registry、Orchestrator、权限门控、OpsAgent、记忆实现 | `core/agentcontract` 等，**不导入任何 Agent 实现** |
...
见[Agent 运行时配置](../operations/agent-runtime-config.md)。默认 `enabled: [task, ops]`；`TANGYING_AGENTS=task` 只启用执行 Agent，用于判断"观测本身有没有改变行为"。

## 与 Python 侧恢复引擎的边界

`robot/gateway/.../fault_remedy.py` 是**执行引擎**：它决定该不该跑一个有界恢复动作、按什么顺序、结果如何，成功判据是复查台账后故障消失。

OpsAgent 是**只读观测者**：它读同一个故障台账（`robot.faults.v1`），把它翻译成结构化发现，然后停下。它不调用恢复引擎，不执行恢复动作。第一版对任何恢复建议都恒为 `automationLevel="advisory"`、`requiresApproval=true`，有测试钉住这一点。

两者的连接点是事件：OpsAgent 输出 `ops.recovery_proposed`，要不要做、由谁做，是既有权限与审批路径的事。
