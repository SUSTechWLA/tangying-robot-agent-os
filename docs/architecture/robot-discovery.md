...
`pairingState` 是机器人的**自述且未经核实**。Agent 不得把它当作授权：它只决定怎么显示。一台机器人能不能被配对，由机器人在收到配对请求时决定——那一步见[机器人配对](robot-pairing.md)。

因此发现协议里没有、也永远不会有配对码：广播是明文未认证的，广播配对码等于删掉"必须有人站在机器人旁边"这个前提。
...
- **配对仍然需要人去读一个码**。这是设计上不能绕过的：你无法认证一个和你没有共享秘密的设备。详见[机器人配对](robot-pairing.md)。
- **配对生效需要重启本机 Local Agent**。机器人凭据在启动时读取；接口会明确返回 `restartRequired`，不会假装已经生效。
