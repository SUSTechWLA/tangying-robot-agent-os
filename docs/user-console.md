...
工作台可切换顶部桌面与底盘前方相机，查看彩色、深度或观测点云。三种模式使用相同大小的画面区域，切换时页面不会忽大忽小。正在读取下一帧时会明确标出暂时显示的上一帧来源与模式；准备完成后再整体切换。数据过期或无效会显示未知。

画面左下角的 FPS 是最近 5 秒实际成功显示的新采集帧率，刚打开或切换时先显示“采样中”；它不代表相机硬件的额定帧率。旁边“画面 N 秒前”表示距离原始采集已经多久。拖动点云视角不会让 FPS 虚增，图像停止更新时帧率会下降。

彩色、深度和点云共用固定大小的画面区域。切换时会暂时保留上一帧并标明来源，等新画面准备好后再替换，避免闪白和页面跳动；相机响应较慢时也会等待当前采集完成。过期的画面仍明确显示不可用。

切到任务记录、其他页面或隐藏浏览器标签，会暂停不需要的画面刷新，返回工作台立即更新。机器人后台任务与安全控制继续运行。相同任务数据不会反复重建按钮和卡片；正在输入的任务、当前焦点以及选中的历史证据会保留。历史回看显示的是选中步骤当时保存的观测，不会被新直播画面覆盖。
...
**Local 单机器人 RGB-D** 提供“彩色画面 / 深度画面 / 观测点云”。彩色用于看物体，深度预览以颜色表达距离，点云只表示本次相机看到的表面；遮挡和视野外保持未知。页面标明来源与采集时间；过期、缺失或身份不一致时清空，不展示成实时环境。历史观测明确标记历史时间，可从任务证据回看同帧 RGB/depth。完整模拟世界仅供开发模式调试。

点击“观测点云”即可查看带相机原色的三维表面，红杯、蓝瓶会保留其 RGB 颜色。默认聚焦桌面工作区，可拖拽旋转、滚轮缩放、点击“复位视角”；开启“显示识别名称”辅助找物品。旧记录或无彩色传感器显示“未含 RGB · 单色”，不会凭名称着色。小物体保留更多实测采样点，但点云仍有空隙，不是补全后的实体模型。

以下是保留的 **Fleet 旧仿真展示**；它的全景和占用图不代表新 RGB-D 路线拥有这些环境信息：
