...
RoboCasa 本地 profile 会自动建立演示操作员会话，页面直接进入控制台；通用 Fleet 和生产部署仍显示登录流程，设备数据面在两种模式下都保持独立认证。
...
输入“让1号机器人把红色方块放到交接区，然后让2号机器人把红色方块放到右侧目标区”并点击创建后，页面先立即显示“正在理解和编排”，无需等待任务 API 返回才有反馈。任务轨道随后固定显示：

1. 准备新的仿真环境；
2. 1 号机器人把红色方块放到交接区；
3. 2 号机器人把红色方块放到右侧目标区。

任务区优先显示“用户说了什么、系统理解成什么、当前做到哪一步”。子任务使用可折叠卡片：正在执行、等待证据、失败或恢复中的卡片自动展开；已经完成的卡片默认只占一行，但始终显示步骤编号、机器人、能力和结果。展开后再按执行顺序查看“查看周围环境、确认目标、规划拿取、拿稳、确认抓稳、放下、确认放好”等人话工具；工具参数放在二级“查看动作细节”中，避免长 JSON 挤坏页面。任务标题限制为两行并保留悬浮完整文本；选中任务后“创建新任务”自动折叠，让普通用户首屏先看到执行反馈。

`正在发送 → 机器人执行 → 等待环境确认 → 环境已确认` 与同一工具的相机 capture、世界 revision 和 Harness 证据同步；工具返回成功但环境/相机没有前进时，步骤只显示“正在确认结果”或“正在安全恢复”，不会提前变成完成。任务更新先展示变化预览；物理工具运行中则显示“等待安全点”，到安全边界才启用新 revision，且不会重置当前 episode。

两张“1号机器人头部 RGB-D”和“2号机器人头部 RGB-D”卡片来自各自 XLeRobot 头部相机。RGB 是当前彩色图；深度是同一 capture 的米制深度 PNG，并提供原始深度链接。卡片同时显示 episode、capture、world revision 和新鲜度。固定的“MuJoCo 全景（环境观察）”由升高并对准双机器人交接工作区的 operator camera 生成，只用于帮助用户看完整厨房，不标成 Harness 相机证据。所有相机图使用完整画面适配，不裁掉边缘；桌面端为全景在左、双路 RGB-D 在右，窄屏自动改为两列或单列。用户可点“放大查看”打开不裁切的模态大图，并可用关闭按钮或 `Esc` 返回。

错机器人、错 episode、倒序 capture、比世界更超前或过期的帧会被拒绝；标签变为“证据等待/已降级”，不会继续显示旧图冒充实时画面。切到后台会暂停媒体刷新，回到前台再恢复；退出会中止请求并释放图片 URL。
