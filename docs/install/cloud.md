...
> **状态：已于 2026-08-18 被本地优先架构取代。** 本文保留用于追溯旧版本部署和迁移决策，不再是可执行安装指南。当前云端安装请使用[Fleet 指南](../architecture/fleet-cloud.md)；离线形态使用[Local Agent 指南](local.md)。历史取代经过见[本地优先设计规范](../superpowers/specs/2026-08-18-local-first-runtime-design.md)，后续又演进为 Fleet 主形态。旧 `cloud` 安装角色只返回迁移提示。

以下内容是 `v0.1.0-rc.2` 的历史记录。
