...
四种场景均含一个可重复验收的彩色工位。默认头部 RGB-D 用颜色连通区域、深度和圆柱表面拟合识别 `red-cup`、`blue-bottle`、`right-bin`、`front-tray`；重名、遮挡、过期反馈会拒绝执行。该探测器只用于已标定的这些物体，不是任意家庭物品识别。底盘 IMU 补偿俯仰/横滚，二维里程计提供平移与航向。底盘相机仍用于建图。
