...
**发布前有一条校验：栅格里的占用格必须能在同一张地图的点云里看到。** 续建的每一段都会把继承来的点云重新降采样一次（点被平均、重新分箱，逐代变稀），而合并栅格会把历史上被告知过的占用格一直保留下来——"占用压过自由、也压过一个点都没有"。实测一次四段从零探索：6494 个占用格里有 1491 个（23%）在发布的点云里已经没有障碍高度的点，**包括卡住厨房目标位姿的那两格——点云在那里显示的是测到的地面**。操作者打开地图是找不到这个"障碍"的。发布时因此做一次对账：占用格没有点云支撑时，同格测到地面就降为自由，什么都没有就退回未知（"这里没人看过"才是诚实答案）。同一张地图实测：占用 6494→5003（正好等于点云支撑数），自由 24994→25722，未知 8112→8875，外墙等真实障碍不受影响。
