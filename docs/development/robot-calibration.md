...
真机标定不能要求用户读关节名、算十六进制偏移。目标交互是一条**带提示的向导**：一步一个动作，用日常语言说清"现在把左臂这个关节转到最左边，然后点下一步"，带进度、可中断续做、结束时给一句总结。

现已具备的地基：

- 契约、严格校验、内容修订号、带 CAS 的原子存储（本页）；
- 分组寄存器表（`motor_layout()` / `gripper_motors()`），编辑器据此渲染"左臂 / 右臂 / 头部与底盘"，并知道每条臂的夹爪是第 6 号舵机；
- 仿真与实机共用同一份文档，仿真侧已能发布并应用，改动能被观测出来。

**尚未实现**：引导式向导本身（步骤状态机、逐关节提示与采集、断点续做、结果总结），以及承载它的运行时 RPC 与控制台面板。真机标定的现场验收也**尚未进行**——本文不声称任何实机标定已经完成。
