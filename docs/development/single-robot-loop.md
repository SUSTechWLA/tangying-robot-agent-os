...
当前操作对应 `v0.3.0`，按 `git clone --branch v0.3.0` 获取准确版本。本文末保留 2026-09-08 的 RGB-D 软件仿真任务、暂停重启、未知动作阻断与历史图像记录；本次离桌导航与长暂停的实际结果见 [v0.3.0 发布记录](../releases/v0.3.0.md)。模块存在、接口可调用、仿真成功和真实设备上线是不同层次的证据。
...
`PixelDetection` 返回实体标识、类别、布尔像素掩码、置信度和属性。掩码必须与输入图像对齐。通用 `RgbdPerception` 将有效掩码中的三维点归约为实体位置，并按上限抽取点集；默认最多 2048 点，参考桌面使用合同上限 4096 点。采样将约一半预算均分给通过检测的物体掩码，其余覆盖剩余有效像素，防止地面和背景占满预算而漏掉小物体。只选择不重复的实测像素，不补出遮挡表面。这里的点集是有界单帧观测，不是稠密地图或已实现的 SLAM。

每个采样像素同时索引世界坐标 XYZ 和原始 RGB，输出 `points[i]` 与 `pointColors[i]`。颜色是 0–255 的 RGB 整数；坐标变换不会改变颜色对应关系。没有有效深度的像素既不生成 XYZ，也不生成颜色点。实机输入必须先完成深度到彩色相机的配准；相同分辨率本身不证明已对齐。
...
启用导航的严格 profile 会在 `resolve_targets` 后插入 `navigation.navigate` 和新的 `observe_scene`，每项目标为九步。导航使用部署标定的接近位姿；到位后再次测量目标。重复目标即使位置已在容差内也必须取得新鲜的地图/定位结果，不能拿上次成功回执替代。未收臂时任何非零底盘速度仍被拒绝。

工具返回成功后，Runner 持久化完成记录，优先保存命令携带的 `evidence_observation` 原始验证输入，并发布 `TOOL_ACTIVITY` 的 `CONFIRMED`；失败工具也可通过 `FAILED` 保存原始证据。`evidenceIds` 只引用归档成功的采集，`receiptObservationId` 保留回执观测编号，命令原始证据要求二者对应。旧适配器未携带原图时才另采集执行后遥测并标为 `post_tool_observation`。UI 明确区分原始验证输入、旧执行后画面和缺失证据；任务最终成功不能补造某一步的验证依据。放置检查需要自由释放后不同时间的三帧、支撑高度与位置稳定；单帧回看不是独立成功证明。
...
| `GET /v1/scene/camera?adapter=mujoco&sourceId={source}` | 一次指定相机采集，原子返回 snapshot 与同帧 RGB/深度 data URL；用于上下相机切换 |
| `GET /v1/navigation/map` | 当前机器人 RTAB-Map 二维占据图、定位和地图版本；未配置导航返回 503，不显示虚构地图 |
...
点云只读取标准 `reconstruction.points`、可选 `pointColors` 和同次 `reconstruction.entities`，核对机器人/传感器/标定、世界坐标、米制单位、新鲜度和有限点值。存在有效颜色时直接显示对应 RGB 像素；投影裁剪与远近排序始终携带原点颜色。缺失或空颜色列表明确标记“未含 RGB · 单色”，畸形颜色拒绝显示。前端不加载默认桌面、推车或相机模型，也不从完整模拟场景补出未观测区域，空白处保持未知。默认按本次已观测工作区和视口比例聚焦，点大小按屏幕尺寸保持可读；名称默认隐藏，开启后优先任务物品与杯、瓶，限制数量并避让。旧仿真图明确显示“仿真调试画面（非机器人感知）”；单独的开发语义诊断不作为机器人感知画面。
...
以下保留 2026-09-08 当轮的软件仿真验收结果，不将其重标为 v0.3.0 离桌导航结果；当前完整回归汇总见 [V1 状态页](../production/v1-release-status.md)。
