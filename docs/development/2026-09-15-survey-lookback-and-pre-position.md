...
- 文档位置：本文件（机制与口径）、`docs/development/2026-09-15-latency-and-recall-goal-upgrade.md`（上一轮的边界与参数）、`compare_destination_policy.py` 的报告里逐次记录 `recallWindowEnv`，**每次运行都要说明它被允许多旧**，否则两臂不可比。
