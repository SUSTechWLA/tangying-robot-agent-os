...
## 三点五、已落地：异常终态自动产出 + 自动扫掠分类（实测）

上一版的两个关键缺口——"要人工按需调用"与"诊断只活在内存/接口里"——已经补上：

**① 异常终态自动写记录**（Go `incidents/` 包 + `internal/localapp` 钩子）

- 任务到达异常终态（可恢复失败、安全失败、取消、以及运行错误）时，本地 Agent 自动写出
  `artifacts/incidents/<taskId>.bundle.json`（`incident.bundle.v1`），内容全是**事实**：任务身份与终态、
  恢复判定（`canResume`/`requiresReconciliation`/`reasonCode`）、环境指纹（软件/目录/地图/标定修订）、
  时间线（含每步参数与错误）、步骤执行历史、证据引用（含 rgb/depth 哈希）、耗时报告（原样，没有就是
  `null`，**从不编造**）。
- **写入是尽力而为且不改变结果**：写失败只记日志，绝不影响任务终态；写临时文件再 rename，读者不会看到半份记录。
- **有界**：默认保留最新 200 份，按修改时间淘汰——一台跑几周的机器人不会把磁盘写满。
- 目录是部署参数：`TANGYING_INCIDENT_DIR`，车队可指向共享卷，让一个诊断流程扫全部机器人。
- 记录里自带一句说明："本记录只包含事实…不修改任何东西"——**它不会假装自己修好了什么**。

**② 分类只在一处 + 自动扫掠**（`scripts/diagnose_task.py --bundle / --sweep`）

- Go 只写事实，**故障族知识仍然只在 Python 一处**——另一种语言里的第二份拷贝必然漂移。
- `--bundle <file>`：把 Agent 写的 bundle 分类成 `incident.v1`（含诊断）。
- `--sweep <dir>`：把一个目录里所有 `*.bundle.json` 批量分类，打印每条的故障族与错误码，
  **未归类的计入返回值并明确报告**（非零退出码可用于 CI/巡检告警）。

**实测（不是演示）**：在运行中的栈上提交一个必然失败的请求（"从客厅出发，去卫生间确认一下环境"，
地图里没有该区域）→ 任务到达 `RECOVERABLE_FAILURE` → **Agent 自动写出 bundle（无需任何手动步骤）**
→ `--sweep` 分类为 `goal_or_localization_unclear`（命中 `GOAL_NOT_CLEAR`），给出 4 条可能根因、
3 项先查操作与覆盖该族的回归测试。全过程未修改任何代码、未动机器人。

## 四、为 AI 自动运维铺路的下一步（按优先级）

1. **耗时统计跨重启持久化**：incident 已经落盘（含故障时刻的耗时快照），但**每次运行的完整耗时序列**仍在内存里。要回答"这周比上周慢了吗"需要把样本按批落盘（JSONL + 轮转）。
...
5. **把扫掠接进例行流程**：`--sweep` 已可用于巡检，但还没有定时任务/CI 作业在跑它；且未归类会返回非零码，可直接做告警。
