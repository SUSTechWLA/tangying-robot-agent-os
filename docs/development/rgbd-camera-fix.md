# RGB-D 相机：底部相机位置修正（已实施）

用户实机检查发现两个问题，都已**用实测确认**。修复本身很小，但它会牵动自滤波与感知阈值，因此**没有直接改进 main**——补丁保存在本目录，实施步骤写在下面。

## 问题 1：底盘相机装错了位置（已确认）

修复前的实测：

| 相机 | 世界 z | 朝向 | fovy |
| --- | --- | --- | --- |
| `head_depth` | 1.185 | 垂直向下 | 70° |
| `base_depth` | **1.535** | 前下方 45° | **150°** |

**底盘相机比头部相机还高**，而且是 150° 鱼眼俯视地面——它既不在底盘底部，也不看机器人前进方向。源码注释也写着它是"front mast, 45 degrees down"，即一根立杆上的俯视相机，与"底部前向深度相机"完全是两回事。

用户要求：**放在底盘靠近地面处，向前拍摄机器人前进的深度图**。

## 问题 2：两台相机不是同一个型号

用户要求顶部与底部**参数一致，尽量用 Intel RealSense D435i**。修复前两者 fovy 分别是 70° 与 150°，是两种不同的传感器。

D435i 深度流为 **87° × 58°**（H × V）。

## 补丁内容（`patches/rgbd-d435i-cameras.patch`）

1. `xlerobot.xml` / `xlerobot_r2.xml` / `xlerobot_handoff_r2.xml`：`head_depth` 的 `fovy="70"` → `fovy="58"`。
2. `rgbd_navigation.py`：`base_depth` 从 `pos=[0.28, 0, 1.50]` + 45° 俯视 + `fovy=150`，改为
   - `pos=[0.30, 0, 0.16]`（贴近底盘底部）
   - 前向 15° 俯角（`xyaxes=[0,-1,0, sin15,0,cos15]`）
   - `fovy=58`
3. `home_scene.py`：新增共享常量 `RGBD_VERTICAL_FOV_DEG = 58`。
4. **tabletop 场景保持逐字节不变**（`[0.185, 0, 0.50]` + 45°），因为它的验收夹具断言了近地射线。

改后的实测：

```
head_depth  58 x 73 deg
base_depth  58 x 73 deg
base_depth world z=0.160, 前向 (0.966, 0, -0.259)
```

## 一个必须说明的差距

**垂直 58° 与 D435i 一致，但水平只有 73° 而不是 87°**，因为我们的帧缓冲是 4:3（320×240），而 D435i 的深度流是 16:9。要拿到真正的 87° 水平视场，需要把帧缓冲改成 16:9（例如 424×240）。这是**帧缓冲选择**，不是相机参数错误，但会让"是否真是 D435i"这句话打折扣。

## 实施结果

**已实施的只有底部相机**，头部相机保持 70° 不变。补丁中"把头部也改成 58°"的部分**已撤销**，原因见下。

实施后的实测：

```
home_task  head_depth=70  base_depth=58   (底部：z=0.160，前向 15° 俯角)
tabletop   head_depth=70  base_depth=100  (逐字节未变)
```

**验证**：`sim/mujoco/tests` + `tests/` **1004 passed / 7 skipped**，`make home-accept` exit 0，`ruff` 干净。

### 过程中修掉的两个自身错误

1. **补丁把 tabletop 场景也改了。** 我原本声称"tabletop 逐字节不变"，但 `camera_fovy = 58` 写成了一行无条件赋值，把 tabletop 的 100° 也改成了 58°——**5 个测试失败里有 3 个是这个错误造成的**（含两条自滤波）。修正为按场景区分后，自滤波立刻全部通过。
2. **头部相机改 58° 会静默破坏家居感知。** 视野变窄后，原本在 x=1.58 的杯子**移出画面**，感知一个都报不出来——症状看起来像检测器坏了，实际是摆放与视野不匹配。我尝试把物体移到 x=2.55 仍未检出，**在上下文耗尽前没能定位**。

### 为什么头部相机没有一起改

把头部从 70° 改到 58° 需要**重新调校整条家居感知链**（工作体积门控、颜色掩码、支撑面检测都按 70° 调过），并且 `xlerobot.xml` 是**与 tabletop 共用**的（直接改它会连带改掉 tabletop 验收）。

**把两件事混在一个改动里，会让两件都难以审查、且都无法独立验证**，所以只交付底部相机这一件已完整验证的修复。头部相机的 D435i 统一是**独立的一项工作**。

## 用户同时提出的另外几项：现已全部完成

本文写于 v0.5.0 期间，其中三条当时未完成；**v0.6.0 已全部实现**，此表已更新，下面的根因分析作为历史记录保留。

| # | 问题 | 现状 |
| --- | --- | --- |
| 2 | 整机标定、SLAM 应在左侧**独立标签** | ✅ 已完成（v0.5.0）。侧栏新增「整机标定」「SLAM 建图」两页 |
| 3 | 稠密地图未用 WebGL 展示 | ✅ **已完成（v0.6.0）**：`web/src/map_viewer.js` 用 `THREE.WebGLRenderer` 为已保存地图渲染独立场景；见[装修家庭演示](../guides/furnished-home-demo.md) |
| 4 | 「动作与结果」回看观测滚到页面底部 | ✅ **已完成**：改为对话框打开（`web/app.js` 的 `revealLocalEvidencePanel`），见[控制台说明](../frontend/console-v1.md) |
| 5 | 导航需语义地图 | ✅ **已完成（v0.6.0）**：注册服务发布 `semantic_navigation`/`semantic_objects`（`sim/mujoco/tangying_sim/semantic_services.py`），`edge/robotclient/semantic_routes.go` 按语义地图解析路线 |

### 历史记录：问题 3 当时的根因（一个没有生产者的契约）

> 以下分析针对 v0.5.0。v0.6.0 通过注册服务补齐了场景身份与地图渲染，故保留作为决策记录。

### 问题 3 的根因：一个没有生产者的契约

排查结果比"实体为空"更根本：

```
grep -rn "model_hash|modelHash|scene_id|sceneId" core/ edge/   → 无匹配
grep -rn "model_hash|scene_id" sim/mujoco/tangying_sim/*.py    → 无匹配
```

**整个代码库里没有任何一方产出 `scene_id` 或 `model_hash`。** 而控制台的场景包校验（`web/src/asset_registry.js::authoritativeIdentity`）要求从**实体属性**里读到这两个值，读不到就抛 `VISUAL_MODEL_IDENTITY_MISSING`。

所以这不是"这次坏了"，而是**这条契约从来没有接完**：控制台期待两个没有任何生产者写入的字段，因此三维数字孪生在当前代码库中**从未激活过**。

再看 `/v1/world` 的实际返回：`robots` 有内容、`sources` 有内容、`entities` 为 0。实体只在感知发生后产生，**而场景身份是世界的属性，不是某个被感知物体的属性**——这本身就是契约设计上的错配。

### 真正的根因：控制台没有仿真所跑场景的三维资产

继续追下去，发现"缺字段"只是表象，**真正缺的是一份资产**：

| | 是什么 |
| --- | --- |
| 控制台**唯一**的三维场景资产 | `robocasa-handoff-v1`（RoboCasa 厨房，`scene.glb` 34MB） |
| 仿真实际运行的场景 | `home-task-rgbd-mobile-manipulation-v1`（四房间家居） |

```json
// web/assets/scenes/robocasa-handoff-v1/manifest.json
{"sceneId": "robocasa-handoff-v1", "modelHash": "13d8da89…"}
// sim/mujoco/tangying_sim/home_scene.py
HOME_TASK_SCENE_REVISION = "home-task-rgbd-mobile-manipulation-v1"
```

**两者不是同一个场景。** 控制台里没有仿真所跑场景对应的三维资产。

### 所以"把 scene_id 填上"是错的解法

如果让家居仿真发布 `scene_id = robocasa-handoff-v1`，三维视图**确实会亮起来**——**但它显示的是一栋不同的房子**。

那是一个**会说谎的数字孪生**：屏幕上是一间 RoboCasa 厨房，机器人实际在四房间家居里抓杯子。这比"三维不显示"危险得多，因为不显示至少是诚实的。

### 正确的修法（明确了，未实施）

1. **把 MuJoCo 家居场景导出为 GLB**（`home_task` 的墙、家具、任务台、收纳盒、四个物体）；
2. 在 `web/assets/scenes/` 下为它建立自己的 manifest，`sceneId` 取 `HOME_TASK_SCENE_REVISION`，`modelHash` 取导出内容的稳定哈希；
3. 仿真运行时在 `/v1/world` 发布携带该身份的实体（这是仍然需要的那层契约改动）；
4. 控制台按身份选择匹配的资产——已有资产不匹配时应当**明确降级**，而不是拿另一份资产顶上。

第 4 条是这次排查最重要的产物：**校验逻辑本身是对的**。它拒绝加载一份与权威身份不符的资产，正是防止数字孪生说谎的机制。**不能为了让三维亮起来而绕过它。**

### 旧记录（修法的早期判断，已被上面的结论取代）

1. **仿真运行时**发布场景身份：可用已有的 `HOME_SCENE_REVISION` / `HOME_TASK_SCENE_REVISION` 作为 `scene_id`，并对模型内容算一个稳定哈希作为 `model_hash`。
2. **决定它挂在哪里。** 挂在某个感知实体上不合适（感知发生前不存在）；更合理的是世界快照里的一个**场景来源条目**，或顶层字段。
3. **控制台**相应地从新位置读取（而不是只认实体属性）。
4. 补测试：模型哈希稳定、场景切换后身份变化、控制台在两种来源下都能激活。

**这是三层改动（Python 运行时 / Go 世界模型 / 前端校验）加上契约决定**，不适合在上下文即将耗尽时开工——改一半会让 `main` 处于"三维仍不可用、但契约已改动"的更难排查状态。



已在浏览器中量到：

```
工作台文档高度 11050 px    视口高度 800 px
#local-evidence-panel 位于 main 的第 11 / 16 个子节点
```

**代码本身是对的**：`focus({preventScroll:true})` 已经挡住了 focus 的隐式滚动，`scrollIntoView({behavior:"smooth", block:"start"})` 之后还有一轮"落点校正"。

**问题在于距离**：从「回看当时观测」按钮到证据面板要跨过大约 **13 个屏幕高度**的页面。在 11000 px 的页面上做平滑滚动，视觉上就是"一下子到了底部"；中间那些与当前观测无关的面板，正是用户说的"空白无效信息太多"。

**因此这不是滚动动画的参数问题，是结构问题**：证据面板不该住在离按钮一万像素的地方。

**正确的修法**：把观测内容**就地展开在按钮所在的行**（inline 展开），或在抽屉/弹窗中打开——两者都**不需要滚动**，也就同时消掉了"中间空白太多"。

**不要**靠调 `block` 取值或加长动画时间来掩盖：那只会让一次长距离穿越看起来更慢，而不是让用户少滚一点。

