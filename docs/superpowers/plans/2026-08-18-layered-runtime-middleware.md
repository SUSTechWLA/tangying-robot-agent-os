...
**Status:** Delivered on 2026-08-18. All task checkboxes and the complete verification matrix below passed on the implementation branch.

**Verification evidence:** `make generate-check`; `go test ./...`; 100 Python tests; `make lint`; `make build`; 21 installer checks; full-process demo reaching `SUCCEEDED`; and simulation acceptance at 30/30 episodes, 18/18 object-matrix goals, zero safety violations.
...
- [x] **Step 1: Write failing bounded queue and event fan-out tests**
...
- [x] **Step 2: Run tests and verify missing packages fail**

Run: `go test ./middleware/... -v`

Expected: FAIL because middleware contracts and adapters do not exist.

- [x] **Step 3: Implement dependency-free contracts and memory adapters**
...
- [x] **Step 5: Commit middleware contracts**
...
- [x] **Step 1: Write failing adapter conformance and restart tests**
...
- [x] **Step 2: Run tests and verify the new adapter path is absent**

Run: `go test ./middleware/sqlite -v`

Expected: FAIL because `middleware/sqlite` is not implemented.

- [x] **Step 3: Move and adapt SQLite implementation**
...
- [x] **Step 4: Rename the domain repository port**
...
- [x] **Step 5: Run repository and adapter tests**
...
- [x] **Step 6: Commit the adapter move**
...
- [x] **Step 1: Write failing tests using fake ports rather than SQLite/channel internals**
...
- [x] **Step 2: Run tests and verify concrete dependencies reject the new constructors**
...
- [x] **Step 3: Introduce narrow Grounder and Invoker dependencies**
...
- [x] **Step 4: Replace the Local App channel with `middleware.Queue[string]`**
...
- [x] **Step 5: Select adapters only in the composition root**
...
- [x] **Step 6: Run application tests and demo**
...
- [x] **Step 7: Commit port injection**
...
- [x] **Step 1: Write failing semantic command mapping tests**
...
- [x] **Step 2: Run runtime/client tests and verify semantic API is absent**
...
- [x] **Step 3: Add stable semantic capability names**
...
- [x] **Step 4: Implement semantic command mapping in robotclient**
...
- [x] **Step 5: Update Runner and Console to semantic provider interfaces**

Runner asks `InfoProvider.Info`; Console's runtime card uses the same interface. No consumer imports generated protobuf.

- [x] **Step 6: Run Go runtime, Runner, Console, and contract tests**
...
- [x] **Step 7: Commit the semantic client**
...
- Create: `robot/gateway/tests/test_runtime_boundary.py`
...
- [x] **Step 2: Run tests and verify semantic models are absent**

Run: `.venv/bin/python -m pytest robot/gateway/tests/test_runtime_boundary.py robot/gateway/tests/test_service.py -q`

Expected: FAIL because backend methods currently use protobuf types.

- [x] **Step 3: Add semantic backend dataclasses and mapper functions**
...
- [x] **Step 4: Migrate Safety Supervisor and direct backend**
...
- [x] **Step 5: Migrate the ROS backend**
...
- [x] **Step 6: Remove the simulator's stale Pair method**
...
- [x] **Step 7: Run Python runtime, ROS source, safety, and simulation tests**
...
- [x] **Step 8: Commit backend decoupling**
...
- [x] **Step 1: Write a failing rule test plus the real dependency-graph contract**
...
- [x] **Step 2: Run the architecture test and verify existing concrete imports fail**
...
- [x] **Step 3: Complete dependency cleanup and add repository behavior checks**
...
- [x] **Step 4: Rewrite current architecture and middleware docs**
...
- [x] **Step 6: Mark the plan delivered and commit**
