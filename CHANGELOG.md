...
同一套代码两次从零探索：一次探明 80%，一次只有 69%（都以 `no_reachable_frontier` 诚实收尾）。人工起始段（第 1 段，无底图）两次分别跑了 356 帧和 77 帧——**这段与本次改动无关**（第 1 段不继承任何底图），差异来自仿真的实时控制时序，也就是说"从零一次扫全"目前是**概率性**的，还没到工程意义上的稳定。这是下一轮的目标。
