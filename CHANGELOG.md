...
### 修 problems 页面"卡死"的报告：先让它**失败可见**，并说明我没能复现

用户报告：problems 页面一点进去就卡死。

#### 一、**我没有复现出来。** 用真实数据测过的路径

| 路径 | 耗时 | 创建节点 |
| --- | --- | --- |
| `problems.js` 的 `render` | 12 ms | 103 |
| 真实横幅渲染（含真实 `renderRecovery`） | 28 ms | 443 |
| 点筛选项 | 0 ms | 7 |

载荷本身也不大：7 个组、41 KB。所以**不是数据量，也不是渲染逻辑里的死循环**。

排查过并排除的：
- 载荷体积（7 组 / 41 KB）
- 筛选点击递归（`render` → `renderFilters` 不回调 `render`）
- HTML 结构（`problems-panel` 与 `onboarding-panel` 同级，标记闭合正确）
- 路由解析（`problems` 在 `pages` 里，`resolveRoute` 能解析）
- 声明顺序 / 暂时性死区（`agentAlertSeverityLabels` 在 5043、赋值在 6131，都在前面；
  `alertHandlingLabels` 虽在后面，但只经 getter 访问，且 5 秒轮询的首次 await
  之后才用到）
- 400 ms 的 `refreshPageData`（路由未变即早退，不重渲染）
- 5 秒的告警轮询（间隔正常）

**结论：我没有证据说它为什么卡。** 不下"已修复"的结论——那会是这一轮我自己的假声称。

#### 二、但我确实找到并修了一个真缺陷：**失败是静默的**

`refreshAgentAlerts` 原本用一个 try 包住横幅和问题页两处渲染：
...
任何一处抛异常，都被同一个 catch 吞掉 → **页面什么都没有**。
而"什么都没有"和"系统健康"长得一模一样——**这是告警界面最不能产生的那个结果**，
也正是这一整轮在修的那类缺陷，只是换了个地方。

改法：
1. **两处渲染分开保护**，问题页失败不再连累横幅
2. 新增 `renderError`：把失败**画在问题该出现的位置**，带上错误消息，并明确写
   **"这不代表没有问题；是控制台没能把问题画出来"**
3. 失败时**清空上一次的问题卡片**——失败横幅下面的旧卡片会被读成当前状态

#### 三、顺带修掉一个真实的无障碍/性能问题

`#problems-list` 原来带 `aria-live="polite"`，而它每 5 秒被整体替换一次——
屏幕阅读器会把全部内容反复重念一遍。**问题页是一个目的地，不是跑马灯。**

#### 四、现在请你帮我做一件事

刷新 `http://127.0.0.1:8897` 再进一次「问题处理」。如果仍然卡：
- **现在页面上会直接显示错误消息**（不再是一片空白）
- 同时请把**浏览器控制台（F12 → Console）的报错**发我

这两条里任意一条都能定位。我先把失败变成可见的，是因为在没有复现的情况下继续猜，
只会浪费你的时间。

**验证**：Go 50 包、gofmt 干净、Web **458 通过**（新增 2：失败可见、失败不留旧卡片）、docs 5。
服务已重启，`problems.js` 实测 200 且 `node --check` 通过。

### 问题处理独立成页：横幅只负责"有事找你"，页面负责"怎么办"

用户问：这么多问题要不要单独一个页面管理。**要，而且原因不是"多"，是职责冲突。**

横幅必须**可一瞥**——它出现在用户正在做的事情的角落，回答的是"现在有没有事"。
它不能同时是工作清单：一个必须可一瞥的东西，做不成一张要坐下来读的表。
之前让它两样都当，结果就是被替换掉的那个状态——任务输入框上方压着 276 行，没人能审，于是没人看。

现在两者分工明确：**横幅说有几个问题并链到页面；页面是真正处理它们的地方。**

#### 页面内容

- **标题**：`7 个问题`，副标题 `2 个需要你判断 · 1 个需要你批准 · 4 个系统可自行处理 · 共 120 条报告`
  **两个数字都在**，因为它们的差本身就是发现：少数几个问题由上百条报告描述，说明系统很吵。
  藏掉任何一个，这个页面都会看起来像丢了数据。
- **筛选**：全部 / 需要我判断 / 需要我批准 / 系统可自行处理，每个带计数。
  **计数为 0 的筛选项是禁用而不是隐藏**——隐藏会让分类集合随着问题来去而变化，读者就无法学会东西该去哪儿。
- **问题卡片**：左边框按处理方式着色（红=需要人／黄=需要批准／绿=自动），
  标题 + 机器码（**标签给人读，代码给人搜**）、影响任务数、最近时间、判定理由。
- **详情（点击展开）**：建议动作、还缺什么、恢复计划（**与工作台同一个 `renderRecovery`**）、
  相关任务链接（点一下把 task id 带进工作台）。

#### 几个刻意的决定

1. **筛选状态存在模块里而不是 DOM 里。** 轮询每几秒重渲染一次，存在 DOM 里会把读者每次打回"全部"。
   横幅可以这样（它不被阅读），这个页面不行。
2. **分组不在前端重算。** 页面直接渲染服务端的 `payload.groups`——
   第二份"什么算同一个问题"的定义，会在 id 格式改变的第一天就和第一份不一致。
3. **标签表只有一份。** 页面通过 `window.tangyingAlertLabels` 读 app.js 的，
   而不是自己抄一份，否则同一个问题在横幅和页面里会被叫成两个名字。
4. **一次请求喂两个界面。** 两次 fetch 可能落在一次变更的两侧，让两个本该一致的屏幕显示不同数字。
5. **读不到 ≠ 没有问题。** agent 不响应时页面明确写"这不代表没有问题"。
   空白页和健康长得一样，那是最糟的失败方式。
6. **点详情后不跳走。** 重渲染后 `scrollIntoView({block:"nearest"})`——
   读者点的是卡片里的按钮，把他弹回页首等于丢掉他正在读的那张。

#### 测试

新增 `web/problems_dom_test.mjs`，**跑真实源码打桩 DOM**（不是正则扫源码——
正则只能抓"某函数不再提到某字段"，抓不到"卡片渲染成空"、"筛选没筛掉任何东西"、
"该有问题的地方出现了空状态"）。10 个用例：

一问题一卡（34 条报告不变成多张）、两个计数都对、判定在卡上不在点击后、
已解决的不列出、筛选真的筛、计数为 0 的禁用而非隐藏、无问题是一个状态而不是空白、
筛掉全部时自我解释、详情默认收起且展开后有推理、禁止重试写在问题旁边。

其中两个验证过"去掉即红"：去掉 active 过滤 → `resolved problems are not listed` 失败；
去掉 handling 过滤 → 两个筛选用例失败。

**验证**：Go 50 包、gofmt 干净、Web **456 通过**（新增 10）、docs 5。
资产已注册进 `web/embed.go`，实测 `problems.js` 200、页面标记齐全。

### 问题列表从 279 条报告变成 7 个问题；修掉三处身份缺陷

用户反馈：工作台一次列两百多条，没有管理能力，人无法审核。

#### 一、根因：列表展示的是**报告**，不是**问题**

三个缺陷叠在一起：

1. **一个问题的三个阶段被当成三个问题。** `ops.anomaly_detected`、
   `ops.root_cause_hypothesis`、`ops.escalation_required` 是同一个发现的三个阶段，
   而 ops agent 给后两者加了 `hyp-` / `esc-` 前缀，投影原样保留 → 一次异常在控制台上是
   三行。参考部署里 92 个问题因此显示成 276 条。
2. **`AnomalyReportID` 的 `#count` 后缀把一个问题切成多个。**
   那个后缀是"**什么时候再喊一次**"（持续存在只喊一次、恶化才再喊），不是"问题是什么"。
   按报告 id 分组，`ANOMALY_ABNORMAL_TASK@task` 被拆成 6 个组（count 25–30）。
3. **升级事件的两行是空的。** 升级把内容放在 `context` 映射和 `reason` 里，
   而投影只读 `code`/`message` → 控制台上是 `code: null` 和字面量 `"None"`。
   **最需要人读的那几行，恰恰是什么都没有的那几行。**

#### 二、修法：`tasks.GroupAgentAlerts`

- **按身份分组**（`code@component`），阶段作为字段而不是新行，取**最远的阶段**展示
- 阶段前缀与 `#count` 后缀**在同一个 `rootIdentity` 里剥离**，
  边界用 `agentcontract.SameAnomaly` 的同一规则，改动只有一个地方会错
- 身份**只在载荷完全没给 id 时**才从 `context.component` 合成——
  之前无条件重建，把 `esc-ANOMALY_ACTION_FAILED@verify_placement` 变成了
  `UNKNOWN_OUTCOME@verify_placement`，**一个不同的问题**，于是升级事件没有并进它所属的异常旁边
- `code` 优先取 `reason` 再取 `category`：`reason` 命名的是**这个发现**，
  `category` 命名的是它被归入的**失败类别**。一行标题写 `UNKNOWN_OUTCOME`
  只告诉人这是哪类问题，不告诉人是哪一个
...
每行带一个由**计划**推出的判定，而不是对问题的看法：

| | 含义 | 依据 |
| --- | --- | --- |
| `automatic` | 系统可自行处理 | 计划里每一步都只读 |
| `approval` | 需要你批准 | 有计划步骤会动机器人 |
| `human` | 需要你判断 | 没有可自动执行的恢复动作 |

判定规则与自动通道执行的规则**是同一条，从另一侧读**。这就是"简单问题无需人工介入"
在界面上的落点：当前 7 个问题全部是 `automatic`。

排序是**审核顺序**而不是日志顺序：活跃优先 → **关于机器人本身的优先**
（下面的问题都是通过一台可能不正常的机器人测出来的）→ 需要人的优先 → 严重度 → 影响面 →
身份兜底（否则等价的两人问题会在刷新之间互换位置，人就找不到刚才那行了）。

#### 四、分组差点丢掉两样东西，都是测试逼出来的

1. **调查轨迹。** 分组后不再渲染"读了哪个库、哪张表"。轨迹属于**某个任务**而不属于组，
   所以取最新成员的那份并在任务链接旁标明——分权不应该成为丢掉推理过程的理由。
2. **机器人级故障的优先级。** 扁平投影本来按"机器人优先"排序，分组把它弄丢了。
   已加 `RobotWide` 并补了 Go 测试：**info 级的机器人故障必须排在 critical 级的任务问题之前。**

#### 五、还有一处真实缺陷：分组行里的按钮会 POST 空 taskId

组的 `tasks` 是列表，而批准执行要一个 taskId。修前会提交空值——接口会接受，
然后把它归到"没有任务"下。现在用组内第一个任务，并在代码里写明为什么这是对的。

#### 六、顺带发现的

`web/app.js` 现在渲染分组；**服务端不发 `groups` 时会回退到按报告列出并明确标注**
（"本地 agent 未提供分组视图"）。空列表和有问题是两种状态，空横幅长得像"系统健康"，
那是最糟的失败方式。

**验证**：Go 50 包、gofmt 干净、Web **446 通过**（52 个 DOM 测试全绿）。
新增 15 个测试：11 个分组（阶段合并、`#count`、handling 三分、排序、上限、边界）、
2 个投影身份（阶段共享身份、计数后缀不切分，**都验证过"改回旧代码即红"**）、
1 个机器人优先排序、1 个渲染。
实测 120 报告 → 7 问题，全部 `automatic`。
...
#### 一、分界线：会移动判据的一律不许是 Agent

| 需要的能力 | 用什么 |
| --- | --- |
| 读账本、统计、导出 | 确定性 CLI（`training-export`） |
| 判断数据够不够 | 量化报告 + 退出码 |
| 判断 checkpoint 好不好 | `train.Compare` / `Gate`（训练方够不到） |
| 判断计划对不对 | `orchestration/eval` + `make nl-eval` |
| 诊断失败运行、提议配置 | LLM（可以是 Agent） |

#### 二、唯一规则：闭环确认过的才算正样本

**"步骤状态是 COMPLETED"不算证据**——这正是 `core/closedloop` 的立场。
训练数据若按"步骤报成功"来建，**教给模型的就是执行层拒绝相信的那个说法**。

`unknown-outcome` **两半都不进**：当成功教它做没人核对过的物理动作；
当失败教它回避"效果未被确认"的动作，而不再出现未确认效果最省事的办法是
**偏好回报快的动作**——那是 reward hacking 的方向。它被排除、计数、写进 manifest。

#### 三、拒绝样本不可能来自账本

**被拒绝的请求不会变成任务**，服务在创建时就拒了，什么都不存。
所以账本里只有"系统已经答应要做"的请求——**恰好是最错的样本**。
只从账本建的训练集教出来的是「每个请求都有答案」，而"知道什么时候该说不"
是蒸馏里最先丢、最不能丢的能力。

所以拒绝样本是**采**出来的（`HarvestRefusals`），语料默认含 eval 里那 6 条必须被拒的用例。

**已知缺口（写在代码里）**：今天不记录被拒绝的请求，所以"拒绝集有多大"取决于
**有人写了多少条**而不是系统见过多少条。补法很小，且没有替代来源。

#### 四、在真实账本上跑出来的结果

```
记录 64 条，不同请求 15 个
verified 23 / unknown-outcome 29 / unfinished 12
deterministic 59 / llm 5
可用正样本 0 条
切分：SFT 0 / 拒绝 6 / 负样本 0
排除：verified-but-no-plan 4、unknown-outcome 3、unfinished 8
```

**"0 条正样本"不是流水线坏了，是它正确地说了实话**：23 条 verified 全是导航任务
且计划由确定性生成（空 bundle，无编排信号）；5 条 LLM 计划的全部没成功（grounding 坏）。
流水线把"先修执行链"直接打在报告里，而不是打印一张好看的表格让人去开训练任务。

#### 五、工具层的训练记录是白送的

`actionloop.Round` 已有 `Candidates`（当时有哪些可选）/`Tool`（选了哪个）/
`Reason`（为什么）/`Verdict`+`Class`（结果与失败分类）——**这就是一条完整的工具选择
训练记录**，且天然带候选集信息，能让模型学会**在什么情况下不选**。

#### 六、写测试时被自己的用例抓到的矛盾

`VerdictUnknownOutcome` 的文档与 `Why` 都写"既不作为正样本也不作为负样本"，
而 `Split` 把它放进了 `Negative`。**文档与实现矛盾，测试抓到了。**
已改为排除并把计数写进 manifest——按文档那条更强的规则，因为它是对的。

#### 七、顺带修掉一个"不可能失败"的就绪探针

`internal/discovery` 的测试有一个多次复现的 flake（本轮之前就存在，单独跑 3 次仍失败 1 次）。
第一次归因错了：我以为是 3 秒超时太紧，把等待放宽到 20 秒——**照样失败**，
说明不是慢而是丢包。已回退那个改动（**一个不成立的解释留在注释里比没有注释更糟**）。

真正的缺陷是就绪探针：

```go
connection, err := net.Dial("udp4", "127.0.0.1:45899")
if err == nil { return listener }   // UDP 没有握手，对没人监听的端口 dial 也返回 nil
```

**这个探针永远成功，因此永远检测不出"还没就绪"**，循环第一次就退出，
第一个跑的测试与 listener 启动赛跑并丢掉自己的广播——**哪个测试失败在每次运行中都不一样，
因为失败的是跑得最早的那个**。改为由探针自己去 bind 并等待它**失败**
（"地址已被占用"是本地唯一能证明别人持有该端口的证据）。单包连跑 5 次全绿。

**仍未解决**：全量并行下该包仍会失败一次，已复现，根因是**测试硬编码了 45899 端口**
导致同类实例互撞——与上面那个探针缺陷是两回事，本轮未修。

**验证**：Go 50 包（新增 `training/`）、gofmt 干净、docs 5。
`internal/discovery` 全量并行下仍偶发失败（见上）。
### TrainAgent 可行性评估 + `train/` 门禁：把"判卷"从"训练"里切出去

问题：**是否该做一个 TrainAgent，统一负责编排层、VLA、RL、深度学习等训练，
用 LLM 自己设计训练目标与过程？**

**结论：可行，但"一切皆 Agent"要收窄一句——一切有独立判据、短周期、可复验的工作可以是
Agent；训练没有独立判据，所以它不能是，除非把判据切出去。**

#### 成立的部分

观察训练运行、诊断失败、提议超参/数据配比、写训练代码与评测用例、管理实验台账——
这些与 `ops` 同构，**合起来就是一个有价值的 TrainAgent**：把训练从散落脚本变成
可观察、可追溯、可复用的过程。与 `task` 做的事是同一件事。

#### 不成立的部分

1. **时间尺度对不上**：本仓库的租约/fencing/新鲜度窗口是为秒级物理动作建的，
   训练 run 是小时到天级。硬套会得到一个看起来统一、实际没有保护作用的架子。
2. **判据不独立**——最根本的一条。机器人闭环成立是因为**世界是判据且不受策略影响**。
   训练里判据是 metric，而 run 正在优化这个 metric：**被测量者与测量者是同一个系统**。
   reward hacking 不是"会不会发生"，它本来就是优化目标。
3. **失败形态更隐蔽**：坏动作有界（审批/门禁/复验，最坏撞一下）；
   坏的上线模型是静默的，可能分数还涨了。**"坏得安静"需要和"坏得响亮"不同的机制。**
4. **为了统一而统一**：`eval`/`experience`/`escalation` 只留接口不实现，已有判例——
   把没接线的机制留着冒充已实现，比删掉更糟。TrainAgent 塞进现有抽象等于宣称它继承了
   那套安全属性，而它结构上继承不了。

**核心一句**：LLM 可以设计训练的**过程**，绝不能定义**什么算更好**——
后者是它唯一也能移动的东西。
...
```go
decision := train.Compare(train.Report{Candidate: ..., Incumbent: ..., Eval: ..., Baseline: ...})
```

1. **Pareto，不是加权分**：可执行与拒绝两半都不许变差、至少一个变好。没有汇率——
   加权分会让"多规划几条"去买"少拒绝几条"，而一个什么都规划、什么都不拒绝的模型
   不是"好模型的 90%"，**它是一个会把否定句当命令执行的模型**。
2. **考试不能改**：拒绝比较用例集不同的两份报告（数量不同、成员不同都拒）。
   不能靠改考卷通过考试。
3. **没测过 ≠ 考砸了**：运行未完成记为 `run-incomplete`，既不上线也不丢弃。
   当成 0 分会因一次掉线丢掉好 checkpoint；当成及格会上线一个从没测过的模型。

首次上线走**另一个**判断（`Gate{MinExecutable, MinRefusals}`，绝对门槛，两半各自设线），
**"没有基线"因此不会变成"什么都能上"**。

门禁读的正是 `orchestration/eval` 已产出的报告——判卷与训练用同一个刻度。

**三个守卫都验证过"去掉即红"**：拒绝不回退 → `TestPlanningMoreCannotPayForRefusingLess`；
用例集一致 → `TestASmallerCaseSetIsRefused` + `TestASwappedCaseSetIsRefused`。

写测试时被自己的用例抓到一个顺序缺陷：没有在役基线时，空基线被报成"用例集不同"
（"基线 0 个用例"），会把读者引向"谁动了考卷"。已改为先判"没有可比较的基线"。

#### 建议的形状

**不要**做成"又一个 Agent"，做成两层：TrainAgent（观察/诊断/提议/写代码/记账）
→ 训练作业（外部进程，独占 GPU）→ `train.Compare`/`Gate`（外部判据，训练方不可触碰）
→ 影子编排 → 放行。**TrainAgent 不在判卷链上**：它可以写用例、提议用例，
但不能改这一轮比较所用的那份。

分级：编排层 SFT/蒸馏**可行性高**（数据与评测已就绪）；编排层 RL**中**（需先修 grounding）；
VLA 后训练**中**；"LLM 自己设计训练目标"**低且不建议**；"自动 backprop 编排"**不适用**
（backprop 是框架的事，把它 Agent 化只是包装）。

**验证**：Go 50 包（新增 `train/`）、gofmt 干净、docs 5。
**已知 flake**：`internal/discovery` 的 UDP 发现用例（3 秒超时）在负载下会超时——
单独重跑 3 次仍有 1 次失败，与本轮改动无关（未触碰该包），但它是真的不稳定，记在这里。

### 编排层评测体系：先有刻度，再谈自训

用户问：**用一个小参数量模型自训编排层会不会更好，配一套 eval + 强化学习做自动更新。**
先把"会不会更好"量出来，再回答。

#### 一、评测体系（`orchestration/eval` + `cmd/nl-eval`）

只问一个问题：**给定这句话，产出的是不是一份会执行的计划、以及该不该拒绝。**
不评抽象的"模型好不好"——那没有刻度。

**拒绝是独立维度，不和规划混算。** 报告永远分两半打印：

```
planner=deterministic  6/10 cases (60%)
  executable: 0/4
  refusals:   6/6

planner=deepseek-flash  9/10 cases (90%)
  executable: 4/4
  refusals:   5/6
```

**两边的失败方向相反**：语法太保守（一条都规划不出来），模型太配合
（把"红色和蓝色方块"一次性规划了 14 步）。若只报一个准确率，
"不敢做"和"做错了"会互相掩盖，而两者需要完全相反的修法。

**运行失败 ≠ 答错**：端点不可达记 `Error` 而不是 `Refused`，退出码 0/1/2 分开。
混起来会让"端点挂了"看起来像"判断力差"，训练循环会因此丢掉一个其实没被评估的 checkpoint。

换一个端点就是全部的接口：`make nl-eval MODEL=... BASE_URL=... API_KEY=...`。
**一个 checkpoint 被接受，是因为它在这套用例上的数字更好，不是因为样例读起来更顺。**

#### 二、评测器自己踩了它在注释里警告的坑

第一版 `navigate-kitchen` 用例断言 `wantSkills: ["navigate_route"]`——
那是**解析层的工具名**，而计划里装的是**技能** `navigation.navigate`。
于是一个完全正确的导航计划被判为失败。

**断言目标，不断言实现**：断言"机器人去了厨房"，不是"计划里出现了某个名字"。
这条已经写进 `cases.go` 的注释，并附上这次失败作为理由。

#### 三、剩下那条失败是产品规则，不是模型错误

`multiple-objects`（"把红色和蓝色方块放到交接区"）模型规划了 14 步把两件都做了。
**这是对这句话的一个合理理解**，它违反的是产品规则"一条指令一个物体，否则先问"。
原始缺陷是**默默选一个**，而模型并没有那么做。

用例保留，`Why` 改写成精确陈述这条规则以及模型如何偏离——
**放宽规则还是约束模型是产品决定**，评测悄悄接受任何一种都等于把它藏起来。

#### 四、诚实的结论：值得做，但不会自动带来判断力

小模型自训在**成本、延迟、格式遵从、离线**上确实更好；域内理解可能追平；
**域外/歧义处理明显更差，而"拒绝的准确性"风险最高**。
用 API 模型的输出做蒸馏，学到的是"总是给出计划"——
**"该拒绝时拒绝"这个能力会在蒸馏中丢掉，而它恰好是最不能丢的**。

所以顺序不可选：**先评测（已建），再蒸馏，最后才是 RL。**

#### 五、RL 现在还不能指望

**仿真里任务跑不完**：grounding 仍拿到 0 个物体（`Observe(streams=["entities"])` 无数据）。
这意味着**基于任务结果的奖励现在全是负的，且与编排质量无关**——
再好的计划也过不了 grounding。在坏环境上跑出来的 reward curve 看起来会很像学习，
**实际是在拟合环境噪声**。

现在能用的只有过程信号（计划是否合法、是否过门禁、是否正确拒绝）。

#### 六、写下来的设计

`docs/architecture/orchestration-post-training.md`：

- **数据来源已在库里**（task_events / task_revisions / step_runs / observation_evidence /
  ops.anomaly_detected），导出成一份 JSONL，SFT 与 RL 共用——
  **两份数据必然漂移**。
- **奖励按可信度排序**：闭环证据（最强，且是防 reward hacking 的关键）> 任务终态 >
  过程合法性（便宜但可刷，单用会训出合法但无意义的计划）。
- **三条硬规则**：拒绝必须能得分（否则训成永不拒绝）／未知结果既不算成功也不算失败，
  而是屏蔽该样本（算成功就是训"敢于制造未知结果"）／**闭环门禁不参与训练**，
  训练只能读它的结论。
- **影子编排**：新 checkpoint 与现役对同一批真实请求各出一份计划，只有现役被执行，
  两份都进评测——唯一能不拿真机冒险就拿到线上分布的办法。
- **六种退化**与其防线（永不拒绝／拒绝一切／计划膨胀／格式坍塌／刷过程奖励／过拟合评测集）。

**验证**：Go **50 包**（新增 eval 包）、gofmt 干净、docs 5。
新增 11 个测试，其中两个验证过"改回旧代码即红"（顺序判定、拒绝维度）。
...
| 次数 | 身份 | 类别 |
| --- | --- | --- |
| 4275 | `ANOMALY_UNVERIFIED_MUTATION@execution` | UNKNOWN_OUTCOME |
| 695 | `ANOMALY_ABNORMAL_TASK@task` | — |
| 7 | `ANOMALY_ACTION_FAILED@resolve_targets` | PERCEPTION |
| 7 | `ANOMALY_ACTION_FAILED@observe_scene` | RESOURCE |
| 6 | `ANOMALY_ACTION_FAILED@verify_placement` | UNKNOWN_OUTCOME |
| 6 | `ANOMALY_ACTION_FAILED@navigation.navigate` | PERCEPTION |
| 4 | `ANOMALY_TELEMETRY_STALE@telemetry` | — |

**所以"139 项"不是 139 个问题，是 7 个问题在 52 个任务上的投影。** 而占绝对多数的那一条
（`ANOMALY_UNVERIFIED_MUTATION`），在目录里对应的唯一动作是 `observe.re-read`——**只读**。
也就是说：**这 139 项里绝大多数，系统本来就该自己处理，而处理方式是"去看"，不是"去改"。**

#### 一、`rule.reconcile-first` 曾经把系统锁死

闭环契约说"结果未知时不得改动机器人"。这条规则原来的实现是 **escalate 且不带任何步骤**，
附一句给人看的话"先观测机器人当前姿态与目标物体位置完成对账"。

读起来正确，跑起来是死锁：**完成那次对账的动作本身就是一个只读观测**，而 escalate 会把步骤全部丢弃。
于是系统能发现"世界状态不明"，却永远无法自己离开这个状态——只能等人。

**契约要求的是"先取证"，取证就是读。** 现在这条规则改为：提出目录里匹配的**只读**动作，不再 escalate。
模型在这一支里**根本不会被咨询**，所以"挡住会动机器人的提案"是结构性的，不是事后过滤。

**边界说清楚**：
- 结果未知时**仍然没有任何改动机器人的动作**会执行——那 7 个 mutating 动作全部是 `bounded_write`，自动通道一个都不碰。
- 目录里没有匹配的只读动作时，**照旧 escalate**（新增测试钉住这一支）。
- 旧测试 `TestReconciliationOutranksAModelPlan` 断言的性质仍然成立，断言从"步骤数=0"改成"模型的 mutating 步骤不在、对账读取在"。

#### 二、自动通道本身：`internal/autorecovery`

```
发现 → RecoveryAgent 出计划 → 自动通道只跑只读步骤 → 结果进任务账本
```

**线画在"目录说它 read_only"这一条上**，不是画在"工具自称不改世界"上。两个问题的答案来源不同：
`MutatesWorld` 是服务对自己的声明，风险等级是目录对"运行这个动作意味着什么"的判断。
读风险等级让"能不能无人执行"只有一个答案，且在那个答案被评审的地方。

**跳过也是记录。** 一个只记"跑了什么"的账本无法区分"系统看过并决定不碰"和"系统从没考虑过"——
而前者正是无人值守的全部意义。同时**跑过的步骤不由自动通道记**：那由执行器记（它掌握轨迹与结果），
两边都记会让一个动作在回放里出现两次。这正是告警投影踩过的"同一事实记两遍"。

#### 三、没有模型时：单工具动作照跑，改动照停

用户决定：**单工具动作由目录直接执行；多工具或会动机器人的动作仍必须有模型。**

`actionloop.SingleToolDecider` 因此不是"硬编码的工具选择"，而是**选择集大小为 1 时不做无意义的依赖**。
它拒绝两件事，都不是形式主义：

- **要参数的工具不调用**。编一个像样的参数发给机器人，比不调用更糟；参数值正是必须有模型或人的那部分。
- **不调用第二次**。第一次没达到效果就停下并说明——定死的自动重试是本系统刻意没有的东西，写在另一个文件里也还是它。

执行器侧的门是 `len(tools)==1 && 工具都不改世界 && 动作不需要批准`。**没有模型时"看"照常，"改"照停**：
一个能发现问题却永远无法调查的系统不是更安全，只是更吵。

#### 四、接自动通道时抓出的四个真缺陷

1. **`Source` 说了假话。** `sourceName()` 只要配置了模型就返回 `"deterministic+model"`，
   而 reconcile-first 分支根本没咨询模型。控制台于是把一份纯规则计划显示成"来自规则 + 模型"。
   改为由**构建计划的那一支**设置来源（`plan.Source`），并加 `sourceOf` 兜底为较弱的声明。
2. **目录声明了一个没有任何部署提供的工具。** `execution.read-history` 一直在目录里、一直被匹配、
   一直是本地工具结构体的一个字段——而组合根从没给它赋值。8 次对账全部以
   "这台机器人没有提供 execution.read-history" 收场。现已接线：读该任务的 `step_runs`，
   把每步的 **capability 与 safetyLevel** 一起带出来——"那个开始后没结束的步骤是物理动作"
   是操作者需要的事实，而它无法从状态单独推出。
3. **任务 id 走上下文而不是参数。** `execution.read-history` 需要知道是哪个任务。若做成工具参数，
   decider 就能指定**另一个任务**的历史；而且工具一旦有参数，单工具决策器就拒绝调用它。
   改成运行时陈述这个事实、工具去读（`recoveryexec.WithTaskID` / `TaskIDFrom`）。
4. **`internal/pairing` 的跨语言测试从来没通过过。** 它调用 `/tmp/python_robot_pair_server.py`——
   一个只存在于作者机器上、从未入库的文件。**一个不可能通过的测试比没有测试更糟**，它把"环境缺文件"
   报成了"代码坏了"。脚本已按仓库规范写入 `tests/pairing/`，测试现在真的在跑，Go 通过包从 48 变成 **49**。

#### 五、实测：在真实数据上跑通

用那份 132MB 真实 `agent.db` 的副本（52 个任务、5429 条 anomaly）跑：

| 动作 | 结果 |
| --- | --- |
| `observe.re-read` | **21 次自动执行成功**（`executed=true`，`operatorApproved` 为空） |
| `execution.read-history` | **6 次自动执行成功**，此前 8 次全部因工具缺失失败 |
| 账本事件 | 27 条 `ops.recovery_executed`，轨迹 `tools.resolved → executed → verification.unavailable` |

**没有重复记录**（修前同一批是 54 条，一个动作两条）。

#### 六、必须说清楚：这轮**没有消除任何一条发现**

跑完之后 169 条仍然 active。这不是失败，是能力边界，而且必须写下来：

**只读对账能取证，不能判定。** `telemetry.read` 读的是机器人**当前**状态，
它无法回答"那一步当时到底完成了没有"；`execution.read-history` 显示该步仍是 `STARTED`。
要让发现真正消解，需要有人依据这些证据判定世界状态，而目录里**没有任何动作做这件事**。

**自动通道现在能做的、也是它该做的，是把证据摆到人面前；它不能替人下结论。**
把"读了证据"当成"问题解决了"，正是本仓库反复防的那类假声称。

另外 `verified` 恒为 false：复验端口在生产组合根里仍是空的（见 [恢复 Agent](docs/architecture/recovery-agent.md) §9.1）。
所以今天的结果一律是"已执行但**未确认**"，这是诚实的那一半。

**验证**：Go **49 包**通过、gofmt 干净、`-race` 干净；新增 30 个 Go 测试。
`TestABoundedWriteIsNeverRunUnattended` 覆盖目录里全部 7 个 mutating 动作，
并验证过"把风险门去掉它就会红"。

### 控制台能批准并执行恢复步骤了（并抓出一个"没执行却报已执行"的假声称）

接上一条：执行通道有了界面。计划里每一步现在各带一个按钮（只读的是"执行这一步"，
`bounded_write` 的是"批准并执行这一步"），点它就走 `POST /v1/recovery/execute`，
把结果、执行轨迹和决策轮次摊在原处。

**每一步一个按钮，不是整份计划一个。** 批准一份计划等于批准"这份计划之后可能变成的任何东西"；
批准一个动作批准的是操作者眼前这一行读到的动作。

**控制台不重复做服务端的判断**：风险等级、工具清单、是否需要批准，全由目录在服务端判一次。
在界面上再判一遍就会产生第二个"这能不能做"的答案，而两个答案会漂移。

**执行之后按钮不再恢复**（"已执行"）：物理动作跑过之后再点一次就是第二个动作，双击不能变成一个动作。
被拒绝的则恢复可用——什么都没发生，改完原因再按是安全的。

#### 然后，在真接口上按的时候抓到了一个假声称

`Result.Executed` 原本在 `loop.Run` 返回后**无条件置为 true**。而"没有配置决策器"时循环
是 `BLOCKED` 收尾、**一次工具都没调用**，却依然返回成功。于是一个什么都没做的运行被报成：

```
executed: true, verified: true, verification: "地图已启用"
```

控制台会把它画成 **"已执行并复验通过"**，并把按钮锁成"已执行"。**这是本仓库最不该出现的一类错误**：
它把"我们什么都没做"说成"机器人动了，而且确认动对了"。

修法是**让唯一知道答案的那一层开口**：`actionloop.Outcome` 增加 `Calls`，在那**唯一一处**真正下发调用的地方自增
（`outcome.Calls++` 紧跟在 `l.call(...)` 之后），执行器改为 `Executed = outcome.Calls > 0`。

- 为什么不在 `recoveryexec` 里从 verdict 字符串推断"哪些 verdict 意味着真的调用过"：那是把派发规则抄第二遍，两份必然漂移。
- **没有执行就不复验**：改前 `Verify` 会被照常调用，于是一个**从未被触碰过的状态**被复验器判为"通过"。
  现在直接返回"没有执行任何动作，因此没有可复验的结果"，轨迹名 `verification.not-applicable`。
- 轨迹在未执行时记 `not-executed` 而不是留空——读者对比两次运行时要能看出哪一次根本没到工具。

**两个新测试验证过"改回旧代码就会红"**：回退后 `TestADeciderThatCannotChooseIsNotAnExecution` 报出
`Executed:true Verified:true ... Trail:[tools.resolved, executed, verified]`，正是上面那段假声称。

**验证**：Go 48 包通过、gofmt 干净；Web 446 通过（新增 12 个：11 个执行 UI + 1 个假声称回归）。
在端口 8895 实测：未配模型时 `observe.re-read` → `executed:false`，轨迹
`tools.resolved → not-executed → verification.not-applicable`，并且仍然列出候选工具
`["telemetry.read"]`——操作者能看到"有什么可选"和"一个都没选"。

### 恢复计划有了执行通道：批准一步，按目录声明的工具执行，结论由复验给

恢复 Agent 一直是**只能提议**的——这是结构性设计（它不持有调用器、任务服务、机器人客户端，
`TestRecoveryAgentHoldsNoExecutionPort` 用反射把这条钉住）。缺的是提议之后的另一半：**谁来执行**。
...
**四条检查，顺序就是安全论证**（`internal/recoveryexec`）：

1. **目录决定能不能跑**：只放行 `read_only` 与 `bounded_write`。
2. **声明的工具必须真的存在**：缺哪个报哪个，而不是发出去让远端失败。
3. **风险决定要不要问人**：只读不问；`bounded_write` 必须批准；**没有批准入口时不执行**——"没法问"不等于"可以"。
4. **执行范围 = 该动作声明的工具**：批准的是"加载一张已保存的地图"，不是"模型认为有帮助的任何事"。

**动作 → 工具的映射声明在目录里，不是执行器里的 `switch`。** 多工具动作（`map.re-survey` 声明了 5 个）
**由模型决定用哪些、按什么顺序**。机器人的服务不由执行器枚举，而是从机器人自己的服务目录建工具表。

**接线的过程中修掉一个默认放行的缺陷。** `Executable()` 原实现是 `Risk != never_automatic`——
于是一个**没有声明风险等级**的新目录条目会被判为可执行，而"要不要问人"恰好由那个字段决定。
改为正向列举（`RiskReadOnly || RiskBoundedWrite`）后，**忘记声明风险等级的后果从"自动执行"变成"拒绝执行"**，
这才是失败方向正确的那一边。

**两种批准来源被分别记录**，因为它们不是同一件事：`approval.operator`（人点了按钮，接口本身即批准动作）
与 `approval.granted`（上游策略端口的批准）。第一版只在走端口路径时才记轨迹，
于是**操作者自己点的那条路反而没有批准记录**——事后追溯时"有人能批准"和"这次确实有人批准了"是两句话。

**实测**（端口 8897，**未配模型**）：`estop.release` → `409` 拒绝；`robot.take_over` → `404` 未知；
`observe.re-read` → 执行成功但 `verified=false`；`map.activate` → 轨迹含 `approval.operator`，
执行体报"没有配置决策器"。最后一条正是设计要的样子：**没有模型时它不编造一次调用**。

**诚实清单（写在文档里，不留成代码注释）**
- **复验端口在生产组合根里是空的**，所以今天每次执行都报"已执行但**未确认**"。这是故意留的：
  一个读不懂效果的复验器比没有复验器更糟，它会把"没看出问题"说成"已恢复"。
- **控制台 UI 还没有按钮**：路由通了，界面没接，现在只能由知道接口的人用 HTTP 触发。
- **执行结果还没写进任务账本**：回放里暂时看不到"某次恢复被执行过"。
- **一个我自己的过度声称**：一度以为 `recoveryexec.Scope` 是必需的第二道边界。把 `Scope` 摘掉后
  **负向测试照样通过**——因为工具列表本身已经界定了范围。这是真冗余，注释已按事实改写，没有留着它装深度。

**验证**：Go 48 包通过、gofmt 干净、`-race` 干净；`tests/docs` 5 通过；Python 844 通过。
...
**① 零 SSH 配对（目标第 2 项）**
- `POST /v1/robots/pair`：输入机器人启动时打印的配对码即可接入。不再需要主机名、SSH 账号、免密登录或对面的 sudo。
- 协议：TCP 45872，长度前缀 + JSON，载荷用 **AES-256-GCM** 加密，密钥来自 **HKDF-SHA256(配对码, salt)**；GCM 的附加认证数据是 `robotId`，所以为 A 机器人封装的请求无法在 B 上重放。
- **配对码永不进广播**：广播是明文未认证的，广播配对码等于删掉"必须有人站在机器人旁边"这个前提。码由机器人打印、一次性、有窗口（默认 900 秒）、错误 5 次后窗口关闭且需在机器人本地重开。
- 已配对的机器人**不会再开配对窗口**（那是一条"不碰它就能换掉它证书"的网络路径），而且是**拒绝**而不是静默跳过。
- 每次尝试都上报 `pairing.window-open` / `pairing.rejected`（带 peer 地址）/ `pairing.paired` / `pairing.window-closed`。配对窗口正是有人会来试的时候。
- 配对成功后机器人进程主动退出（退出码 1，`Restart=on-failure` 拉起）以切换到 mTLS——**不这样做就会"看起来配好了"**：证书在磁盘上，机器人还在按老身份服务。
- Agent 侧返回 `restartRequired`，因为机器人客户端是启动时用当时证书构造的。这是本地重启，不是机器人侧配置步骤。

**② 不发明密码学，并把它证明出来**
- HKDF 自己实现（`x/crypto` 未 vendor，加依赖比这更大），因此**同时**用 **RFC 5869 A.1–A.3 官方向量**和 Python `cryptography` 的实现钉住。
- 跨语言契约 `tests/contract/robot_pairing.json`：Python 断言逐字段复现，Go 断言能解开其中的请求与应答，并解出**相同的派生密钥**（一个字节之差会产出"能编译、能运行、永远配不上"的实现）。
- `internal/pairing/crosslang_test.go`：Go Agent 与**真实 Python 机器人**在真 socket 上完成一次完整配对。

**③ 证书改为 Go 原生签发**（`crypto/x509`，不再 shell 调 openssl）
- 一个按钮没法合理 shell 出一串需要临时目录和五个子进程的脚本，失败时还会在磁盘上留下签了一半的证书。进程内签发意味着要么产出整套材料，要么什么都不改。
- 形状与脚本完全一致（ECDSA P-256、CA 10 年、叶子 90 天、DNS+IP SAN），签发后立刻用本端 CA 验一遍。已有 CA 不会被静默替换；只剩一半 CA 时停下并说明缺哪个文件。

**④ 自然语言配置热生效（目标第 4 项）**
- `tasks.Service.SetParser` + `localconfig.Settings.WithOnChange`：保存模型配置后**重建解析器并热替换**，不再要求重启。
- `RestartRequired` 变成**兜底答案**：只有真的有回调应用了变更才清零。没人应用却声称已生效，比让用户重启更糟。

**⑤ 两个只有真跑才会暴露的缺陷（都已修 + 加回归）**
- **`enrollmentPort` 被接受但被忽略**，而配对端口并非总能假定。现在机器人**在广播里公布配对端口**（仅窗口开放时），Agent 依次取：请求指定 → 广播公布 → 协议默认。第一次端到端跑就是因此报"机器人没有开放配对"而机器人其实在等。
- **保留配置的写入器会破坏保留的行**：它按"键"重建文件，写出一个没有值的 `AGENT_PROVIDER` 再把真行追加到末尾。文件里**确实含有**正确字符串，所以"包含即通过"的断言放它过去了。现在整文件比较，并断言注释与空行也在。已验证改回旧写法即失败。
- 附带：`_start_enrollment` 在证书目录未知时会以相对路径建 `./pairing/pairing-code`（测试跑完仓库根多了一个目录）。现在直接拒绝开窗。

**⑥ Web UI**：新增「发现的机器人」面板——列出广播中的机器人、显示配对状态、就地填码配对；没东西可显示时整块隐藏。`listening=false`（没人在听）与 `mismatched>0`（有机器人在广播但版本读不了）分开说明。

**验证**：Go 46 包、gofmt 干净；Web 434；Python 658（含 `robot/gateway`、`tests/contract`、`tests/docs`、`tests/deploy`、`tests/install`）。关键回归均验证过"改回旧逻辑会失败"。
**实测**：真实 Python 机器人 + 真实 Local Agent，无 SSH、无端口提示，完成配对；机器人落盘三个文件（key 0600）、配对码作废；Agent 侧写入五把 `ROBOT_*` 配置且注释与其它键原样保留。
