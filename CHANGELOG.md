...
2. **厨房停靠点是"贴着包络"选的，不是"留余量"选的**——这是这一轮最后卡住的地方，而且**软件修不了**。实测（把机器人放回该位姿，用驱动自己的包络函数量）：

   | 房间目标位姿 | 最近的环境几何 | 距离 | 驱动包络 0.40 m 的余量 |
   | --- | --- | --- | --- |
   | **kitchen (2.05, 3.00)** | `home_task_table` 桌面 | **0.405 m** | **0.005 m** |
   | living_room | `living_table` | 0.450 m | 0.050 m |
   | bedroom | `bedroom_bed` | 0.650 m | 0.250 m |
   | home_corridor | 走廊墙 | 0.720 m | 0.320 m |
   | bathroom | `bathroom_sink` | 0.856 m | 0.456 m |

   厨房停靠点离任务桌真实桌角只有 **0.405 m**，而驱动自己的净空包络是 **0.40 m**——**余量 4.8 毫米**。路由器更松（0.32 m），所以地图说"可通行"，驱动说"会碰"，任务就停在 `NAV_MODEL_COLLISION`。这张桌子同时是抓取工位：杯子在 0.472 m 处、已在臂展边缘，所以停靠点不能简单后移。**结论：这是任务书层面的取舍**——要么按"包络 + 余量"重新选停靠点（并把抓取改成走到位后再伸手），要么扩大机械臂工作范围，要么用标定过的理由明确放宽这一处驱动的净空要求。软件单方面消不掉它。
...
顺带把这条做成了可诊断的：`NAV_MODEL_COLLISION` 现在会带上最近的环境几何名和实测距离（`_nearest_envelope_obstacle`），而不是只说"模型预测会接触"——上表就是这样量出来的，操作者也能自己复现。
