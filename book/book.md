...
> 全书合订本（单文件通读版）。分章版本见 [`book/chapters/`](chapters/)。
>
> 第 1–16 章及附录 A 的数字为 **v0.6.0/v0.7.0 写作快照**（原基线提交 `774bd2a2f`，原统计提交 `8b9683be8`）；第 17 章补记 2026-09-25 云端/边缘 Agent Harness 升级，对应提交 `50cf3c4a9`、`45758e8b6`。旧数字不代表当前 HEAD。
>
> **每个技术断言都可回溯到源码；推断部分显式标注「推断」；文档与代码冲突处显式标注冲突并给出代码事实。**
...
- **一个可以照着做的施工路径**：第 14 章是从零搭建的完整路线，分阶段、有验收标准、有"这一步做完了怎么证明"。

### 不会读到

- **未经实测的性能基准**。项目作者自己说了："分层带来的延迟开销、跨层调用的 p99，本文没有数据。"这本书也不会替他编。第 15 章讨论仍需做的实验；第 17 章记录云边升级的软件证据与目标硬件门禁。
...
这本书里没有魔法。你会看到项目把"让机器人把活干完"这件事拆到能验证的粒度：**一个证据、一个失败码、一个租约、一个版本号地拆。**
...
至于未来，第 15 章列出了这套系统**自己承认**还有哪些没解决；第 17 章补记云边升级后哪些问题仍须到目标环境认证。
...
它可以知道自己"发出去了"，可以知道"对方收到了"，甚至可以知道"对方的执行器动了"。但它**无法知道世界变成了什么样**——因为世界不在它的观测范围内，世界在物理空间里。
...
很多人以为"分布式机器人系统"是为了"扩展性"或"算力"。不是。分层是被三条物理约束**逼**出来的。

### 约束一：控制回路 100 Hz – 1 kHz

机器人的运动控制回路——读编码器、算力矩、下发 PWM——跑在 100 Hz 到 1 kHz。

换算成时间预算：

| 频率 | 周期 | 一个 LLM 推理的时间预算能装下多少个周期 |
| --- | --- | --- |
| 100 Hz | 10 ms | **一个都装不下**（典型 LLM 首 token 延迟 300 ms–3 s） |
| 1 kHz | 1 ms | **一个都装不下** |

这不是"慢一点也能跑"的问题。控制回路不能在两个周期之间停下来等一个网络往返——**它停下来，机器人就失控了。**

**推论：任何把 LLM 放进控制回路的架构都是错的。**

这个项目里，模型选的是**工具**（`navigation.navigate`、`manipulation.pick`），低层动作来自 Policy Provider（VLA / 模仿学习 / RL）。这条边界在 `docs/architecture/llm-driven-execution.md` 有完整论述。

**教学要点**：第一次听到这个数字的研究生，最常问的是"那用更快的模型行不行"。答案是：**不行，因为你不能要求一个概率系统在 1 毫秒内给出确定性输出**——即使推理时间是 0，采样不确定性还在那里。控制回路需要的是**可证明的实时性**，不是"平均很快"。

### 约束二：急停是毫秒级的

> **不能等网络——等得到的那不叫急停。**

这句话是项目文档的原话（`docs/architecture/why-distributed.md:27`）。

急停的语义是："在这个时刻之后，机器人在物理上不再产生新的动量。"如果急停要经过"浏览器 → 云端 API → 云端协调器 → 边缘 worker → gRPC → runtime → 网关 → 控制器"这样一条链路，那么链路上**任何一环慢 100 毫秒，急停就慢 100 毫秒**。

而对于一个以 0.5 m/s 移动、质量 30 kg 的移动底盘，100 毫秒是 5 厘米——听起来不多。但如果它面前是一只手呢？
...
不是"上传太贵"这么简单——是**延迟和带宽的组合让人无法接受**：一辆家用 Wi-Fi 网络的上行通常是 10–50 Mbps，也就是 1.25–6 MB/s。90 MB/s 的数据要传 15–70 秒。
...
> 不是"分布式更好"，是"**不这么做就不成立**"。
>
> —— `docs/architecture/why-distributed.md:30`
...
**这就是分层的全部论证。** 它不是"我们觉得分层比较优雅"，是三条可测量的物理事实把架构挤成了这个形状。
...
| **云端** | **多机**：机队派单、跨机审计、跨机学习 | 不是"算力更大"；**云端不直接控制硬件** |

（`docs/architecture/why-distributed.md:36-42`）
...
真正的理由是：**控制回路和急停不能穿过任何异步边界。** 这跟"边缘节点算力够不够"无关——即使边缘节点有 A100，控制回路**仍然**必须在控制器里。
...
项目文档自己写了反面（`docs/architecture/why-distributed.md:46`）：
...
而"复杂度的真实代价"在这个项目里被逐条列出了（`docs/architecture/why-distributed.md:127-133`）：
...
项目文档给了一张判断表（`docs/architecture/why-distributed.md:211-217`），我把它保留原样：
...
>
> —— `docs/architecture/why-distributed.md:107-110`
...
| --- | --- | --- | --- |
| 协调器 / claim 协议 | 有 | **刻意没有** | `internal/localapp/app.go:1-3` 包注释：*"It **deliberately has no distributed claim or task-lease protocol**: SQLite is the business-state authority and one worker serializes physical work."* |
| Outbox / 事件日志 | 有 | **建了表但不用** | `middleware/sqlite/fleet.go:14-59` 建了四张协调表，但 Local Agent 不构造 outbox 消费者 |
...
通用 coding agent（Codex / Claude Code 那一类）的"重试"是**免费的**：

```
运行测试 → 失败 → 改代码 → 再运行测试
```

重跑一次测试，除了 CPU 时间，什么代价都没有。文件系统是可逆的——`git checkout` 就回到原点。
...
（`docs/architecture/why-distributed.md:80-93`）

### 一张完整的对照表

项目文档给了一张对照表（`docs/architecture/why-distributed.md:66-77`）。我把它逐行加上代码事实，因为这张表是本书第 16 章的骨架：

| 维度 | Codex / Claude Code | 机器人 Agent 运行时 | 代码事实 |
| --- | --- | --- | --- |
| **世界** | 文本文件 | **物理世界** | — |
| **观测成本** | 免费，想要多少次都行 | **昂贵，有时不可观测** | 双 RGB-D 90 MB/s，且被遮挡就是看不见 |
| **动作可逆** | 几乎总是 | **常常不可逆** | `core/closedloop` 的 `UnknownOutcome` 类 |
| **失败代价** | 重跑一次 | **撞坏东西 / 伤人** | 不做软件急停替代实体急停 |
| **重试** | 默认安全 | **结果未知时禁止重试** | `Retryable()` 只对 `Transient`/`Perception` 返回 true |
| **验证** | 跑测试 | **新鲜证据 + 闭环门禁** | `Gate(declaration, dispatchedAt, evidence)` |
| **真相在哪** | 文件系统即真相 | **世界是真相，只能被观测** | `worldmodel.Snapshot` 的每条事实都带 `EvidenceRef` |
| **会话长度** | 分钟到小时 | **年** | SQLite 增量迁移、`mapRevision`、标定版本 |
...
Codex 面对一个歧义，可以问用户。用户就在那里，一秒后就能回答。

一台在家里干活的机器人，遇到歧义时**没有人可以问**。所以它必须：
...
这个项目叫 "Robot AgentOS"。它的作者明确反对这个名字，而反对的理由本身就是一堂架构课：
...
| 操作系统要管什么 | 谁来管 |
| --- | --- |
| **调度** | 机器人控制系统（RT 内核 / ROS 2 executor） |
| **内存** | 机器人控制系统 |
| **驱动** | 机器人控制系统 |
| **实时性** | **机器人控制系统** |
...
**建议的命名**（`docs/architecture/why-distributed.md:197-204`）：
...
1. **工具返回成功，只是"去看世界"的触发器，永远不是"世界已经按预期改变"的证明。** 这不是工具实现得不够好，是结构性的——发起动作的一方无法观测世界。

2. **分层是三条物理约束逼出来的**：控制回路 100 Hz–1 kHz、急停毫秒级、感知每秒数十 MB。但**"分布式"本身不是价值**——价值是闭环契约、失败分类、证据与回放，这三样在单机上同样需要。

3. **最关键的差异是"重试的代价"。** 通用 coding agent 重试免费，机器人重试可能是"把做过的事再做一遍"。这一条差异派生出了这个系统里所有不直观的设计。
...
- **C 还差什么**：
  ...
  - **时间比较的粒度问题**。runtime 以 Unix 毫秒上报，亚毫秒顺序**原理上不可观测**，所以正确的比较是"整毫秒截断后比较"，而不是直接比浮点时间戳。
...
| 内容 | 位置 |
| --- | --- |
| 三条硬约束与分层论证 | `docs/architecture/why-distributed.md:20-51` |
| 与 Codex 的对照表 | `docs/architecture/why-distributed.md:55-93` |
| 真正有价值的三样东西 | `docs/architecture/why-distributed.md:97-112` |
| 分布式的代价清单 | `docs/architecture/why-distributed.md:127-133` |
| 关于 "OS" 这个词 | `docs/architecture/why-distributed.md:189-204` |
| 判断表 | `docs/architecture/why-distributed.md:209-217` |
| 工具结果是触发器的定义 | `core/closedloop/closedloop.go:1-16` |
| 八类失败分类与 `Retryable` | `core/closedloop/closedloop.go:48-68,324-340` |
| Local Agent 刻意无 claim 协议 | `internal/localapp/app.go:1-3` |
| 刻意的非目标 | `README.md:283-291` |
| LLM 与 Policy 的边界 | `docs/architecture/llm-driven-execution.md` |

---

**下一章**：第 2 章 架构总览与演进史 —— 这套系统是怎么长成现在这样的？五个关键转折点分别放弃了什么？
...
**上一章**：第 1 章 物理世界的三条硬约束 · **下一章**：第 3 章 闭环契约：为什么「工具返回成功」不算完成 —— 一次声称成功的物理动作，凭什么被记为完成？
...
**上一章**：第 2 章 架构总览与演进史 · **下一章**：第 4 章 世界模型：当「真相」只能被观测 —— 如果完成需要证据，那么证据描述的那个「世界」是什么？它为什么必须有版本号？
...
**上一章**：第 3 章 闭环契约 · **下一章**：第 5 章 工具层 —— 28 个工具、5 级安全标注，模型能碰什么、绝对不能碰什么？
...
**上一章**：第 4 章 世界模型 · **下一章**：第 6 章 多 Agent 运行时 —— 为什么不能只做一个万能 Agent？"只提议不执行"如何在代码里保证？
...
**上一章**：第 5 章 工具层 · **下一章**：第 7 章 任务编排 —— 从一句中文到 12 步计划，每一步在哪里发生？为什么"确定性后备"是工程必需？
...
**上一章**：第 6 章 多 Agent 运行时 · **下一章**：第 8 章 分布式不是部署姿势，是故障假设 —— 租约、fencing、Outbox，以及六个拒绝点。
...
**上一章**：第 7 章 任务编排 · **下一章**：第 9 章 本地单机形态 —— 砍掉几乎所有分布式机制之后，剩下的那三样是什么？
...
**上一章**：第 8 章 分布式不是部署姿势，是故障假设 · **下一章**：第 10 章 仿真、真机与 sim2real —— 为什么一个项目要同时有三个仿真后端？覆盖率 38.9% → 81% 是怎么做到的？
...
**上一章**：第 9 章 本地单机形态 · **下一章**：第 11 章 可观测性、事故诊断与自动恢复 —— 事故记录、177 个故障码、以及「沉默被读成健康」。
...
**上一章**：第 10 章 仿真、真机与 sim2real · **下一章**：第 12 章 评测体系与后训练 —— "先有刻度，再谈自训"，以及三轮实验的真实数字。
...
**上一章**：第 11 章 可观测性、事故诊断与自动恢复 · **下一章**：第 13 章 安装、部署与运维 —— 角色安装、一键配对、端口差异与三份检查清单。
...
**上一章**：第 12 章 评测体系与后训练 · **下一章**：第 14 章 从零搭建一套分布式机器人 Agent 系统 —— 施工图：分阶段、有验收标准、有"这一步做完了怎么证明"。
...
**下一章**：第 15 章 未来方向 —— 这套系统自己承认还有哪些没解决？24 条显式 TODO 与 6 个推断方向。
...
**上一章**：第 14 章 从零搭建 · **下一章**：第 16 章 机器人 Agent 与通用 Coding Agent 的完整对照 —— 全书二十条差异的总表。
...
**上一章**：第 15 章 未来方向 · **后续增量篇**：第 17 章 云端与边缘 Agent Harness · **附录**：附录 A 源码地图 · 附录 B 检查表 · 附录 C 实验数据索引
...
> 本附录给出**原书写作时**每一类东西放在哪个目录，以及当时实测到的规模数字（附测量命令）。2026-09-25 云边升级新增 `internal/agentharness`、`internal/modelroute`、`deploy/edge-orin` 和统一 Agent 镜像；现行角色与证据见第 17 章。以下历史数字不代表当前 HEAD。
>
...
| **4** | **时间判据的粒度是什么？** | "比时间戳" | **"整毫秒截断——分辨率是物理限制，不是容差"** |
...
| **16** | **这个数字有确定的正?常值吗？** | `Dropped()` 应为 0；"队列深度 500"不是 |
...
**这张表是放行判断的基础。原文（`docs/operations/production-readiness.md:7-14`）：**
