...
> 若5个种子可作为独立同分布伯努利试验且零失败，单侧95%失败率上界约45.1%；仅使用不同种子不证明独立性。
...
在独立同分布伯努利试验假设下，5次零失败对应单侧95%上界约45.1%。这不是“失败率有95%概率小于该值”的贝叶斯结论，也不能直接套用到有关联的重复轨迹。
...
看第二行：**+6.67 pp，CI [+0.83, +14.17]** —— 这个区间不包含0，但不能无条件当作家族校正后的显著性证据；还须核对区间方法与检验口径。

**但 Holm 校正后的 p = 0.132812，不显著。**

**为什么？** 因为做了多次比较。原文的处理是：**不因为 CI 不含 0 就宣称显著**。

**这个纪律比那个结论更重要**：一个做了 3 次比较的研究，它的显著性阈值必须收紧。若三个零假设都成立、检验独立且各自误报率0.05，至少一次误报概率为1−0.95³≈14.26%；存在相关性时该数值不能照搬，联合上界为15%。
...
**本轮完整性因素的点估计远大于语法因素；不能将两项估计的比值外推为通用规律。**
...
> **这 2.19 个百分点的准确率，值不值 约7倍的危险提议率（点估计），并非实测事故风险倍数？**
...
| # | 误解 | 纠正 |
| --- | --- | --- |
| 1 | "上下文表达方式（JSON vs 自然语言）是关键" | **语法主效应在两个模型上都不显著**（+0.55 / +1.17）。**"删掉必要字段"是压倒性的**（+73.55 / +65.51）。本轮完整性因素的点估计远大于语法因素；不显著不等于语法影响为零 |
| 2 | "准确率最高的策略就是最好的" | 分环节策略准确率 **87.19%** 但危险提议率 **2.19%**；统一标注混合 **85.00%** 而危险提议率 **0.31%**。**多 2.19 pp 准确率，代价是 约7倍的危险提议率（点估计），不是实测物理风险** |
...
1. **被评估者不能在同一轮自行修改最终判据。** LLM可提议指标，但最终定义、留出数据、授权和版本应由独立可信评测流程控制，避免自我评分。

2. **三轮实验最硬的结论**（12,484 个请求 / 31,716,142 tokens / 0 协议失败）：**语法主效应两模型均不显著；排列效应点估计符号相反但未显著；信息完整性 +73.55/+65.51 pp 压倒性。** **规划环节在全部候选上 0.00%。**

3. **报告自己算出功效不足**（每环节 n=4，最小非零 p=0.125；原界与独立采样假设下，**4,615个独立族**为界≤5 pp的保守充分估算）。在独立同分布的5个伯努利试验、零失败条件下，单侧95%失败率上界约45.1%；种子是否构成独立试验需另外论证。

4. **分环节策略准确率最高但危险提议率是统一策略的 7 倍。** 该取舍应由有权限的产品、安全与现场负责人基于风险及任务证据决定；提议率不等于事故率。
...
1. **语法主效应在两模型上都不显著**；
2. 排列效应点估计符号相反，但不能据此断言真实效应或模型间差异；
3. 在语法、排列、标注三个表达因素中，只有Pro标注项通过本轮Holm门槛（+5.08 pp）；完整性项另行显著；
4. **"删掉必要字段"是压倒性的**。
