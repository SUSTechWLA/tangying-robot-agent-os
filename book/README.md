...
书中的全部技术事实来自一个真实的开源项目：**躺营（Tangying）Robot AgentOS**，一个把大模型规划、任务编排、世界裁决与机器人实时执行拆成分层运行时的分布式机器人 Agent 系统。版本基线 **v0.6.0**。
...
**教学用**：每章末尾的"教学要点"与"练习题"可直接作为课程单元。
...
同时也说清楚这本书的边界：它描述的是一个**单机器人主线 + 已实现但未大规模验证的多机扩展**的系统。凡是"已实现"与"已验证"的区分，书中一律显式标注——因为把"仿真通过"当成"实机可用"，是机器人领域最常见也最危险的错误。
...
> **⚠️ 关于基线版本的一条重要说明**
>
> 这本书的写作基线是 **v0.6.0**（提交 `774bd2a2f`）。但在研究与写作期间，仓库**升级到了 v0.7.0**（提交 `8b9683be8`）。
>
> 书中所有技术断言都经过实测复核；`VERSION` = `0.7.0`，但 **v0.7.0 标签尚未创建**——处于"已准备、未打标"状态（`docs/releases/v0.7.0.md:3` 自己写明了这一点）。
>
> 下表是**在 v0.7.0 工作区实测**的数字。
...
同理，**不要抄 README 里的测试数字**。README 写"1079 个 Go 测试函数 + 1656 个 Python 测试函数"，而实测值是：
...
（差异来自"统计口径"和"工作区是否有未提交的新测试"。**上表用工作区实测值**，因为那是你 `git clone` 之后能看到的东西。）
