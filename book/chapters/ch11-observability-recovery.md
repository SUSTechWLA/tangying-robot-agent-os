...
软件系统也可能静默失败、需要人工队列、记录跳过决策并审计每一步。机器人恢复增加了持物、受阻、现场人员与实体停止等具体处置条件。

| 责任 | 本项目的落实 | 需核验的证据 |
| --- | --- | --- |
| 故障可见 | 任务状态、告警、未确认步骤与退化健康 | 不能仅凭进程存活或没有日志判健康 |
| 人工介入 | 对账与审批保留明确队列、操作者和理由 | 现场观察、权限和处置结论 |
| 恢复准入 | 恢复目录、风险等级和审批 | 当前版本的执行接线；历史缺口不作为现状断言 |
| 恢复验证 | 重新取得与恢复动作关联的观测 | 工具自述完成不替代后置条件；不能复用旧成功 |
| 审计 | 任务、步骤、命令、证据和恢复关联 | 拒绝、跳过、未考虑应能区分 |
| 角色健康 | 运行时 `Health()` 表达 `DEGRADED` | 观察者必须实际启动、接线并能取得数据 |

字段白名单约束角色权限，健康状态负责报告角色退化；前者本身不能检测角色有没有运行。当前恢复路径是否已闭环应逐条核对代码与测试，不能把旧章节中“还没接线”的实验结论外推到全部当前恢复能力。
...
1. **`blindRadiusM` 一个常数承担两个职责**（第 10 章）——建模盲区 + 阻止追自己的脚印，在所述测试户型中产生相反的覆盖率变化；
