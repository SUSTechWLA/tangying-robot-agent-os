...
| 5 | **状态机的形状由物理决定** | `UNKNOWN_OUTCOME` 是一个**没有自动出边**的状态。软件部署等外部副作用也可能需要未知结果状态 |
...
（部署风险）如果多实例使用了仅限单进程的内存选主实现，它不能协调跨进程领导权。必须核验实际部署的选主后端、任务CAS与执行fencing分别保护什么；不能因单个机制存在就宣布无脑裂，也不能忽略其他保护机制。

**教学要点**：这是"降级路径必须显式"的一个反例。一个更好的设计可能是：启动时如果检测到"没有分布式租约后端"，就拒绝以多实例模式启动，或者打印一个**很响的警告**。

---

## 8.12 分布式责任：共同机制与物理边界

Coding Agent 若调用远程服务、执行部署或运行多实例，同样会遇到部分失败、租约、未知结果与重试预算。数据库事务和文件原子替换都不能消除跨系统部分失败。

| 责任 | 本项目的落实 | 保证范围 |
| --- | --- | --- |
| 观测 | `worldmodel.Snapshot` 保留证据身份、来源、序列和采集时间 | 世界只能通过有限传感器观察，记录字段不自动证明采集可信 |
| 所有权 | 世界写入、任务领取、物体操作分别保护 | 必须分别核验受保护资源，不能把任务 CAS 当成物理单写证明 |
| 未知结果 | 不将 `StatusUnknownOutcome` 自动放回 `READY` | 租约到期表示持有者停止报告，不表示机器人没有执行 |
| 有限重试 | Worker 的 `PolicyMaxAttempts` 限制策略请求重试 | 推理尚未下发动作的重试与物理动作重放不同 |
| 失联策略 | 按部署形态、任务和能力缩小可用范围 | 不能承诺任意断网下继续全部任务，确定性解析也不是通用任务回退 |
| 持久状态 | checkpoint、journal 与版本迁移 | 已有崩溃用例不证明多年运行或任意掉电恢复 |
| 缺失证据 | `WORKER_REPORT_NO_WORLD` 显式记录；不可验证的场景改变 intent 被拒绝 | 命令回执不能补出真实世界的缺失证据 |

边缘 `edge/recovery/classifier.go` 将观测等待和策略请求失败标为可重试，它与 `core/closedloop` 的物理结果分类负责不同边界，不能只凭 `Retryable=true` 再发抓取命令。

### 谁提供证据、谁判断完成

工具可以返回证据引用。是否接受应取决于采集来源、命令关联、新鲜度、后置条件和判定权限，而非“一切工具提供的证据都不可信”。可信 Runtime 元数据决定是否启用证据门，不能单独证明传感器数据未被伪造。同毫秒时间桶的局限见第3章与附录D。
