...
## 4.8 五项世界建模责任：从文件状态到物理观测

文件系统也会并发变化，路径还受符号链接、容器挂载和命名空间影响。两类 Agent 都需要判断状态的来源与有效期；机器人观测额外面临遮挡、采集延迟和身份关联的不确定性。

| 责任 | 本项目的落实 | 不能据此推出什么 |
| --- | --- | --- |
| 新鲜度 | 事实携带 `Freshness`、`Confidence` 和证据；过期事实不可当作当前真相 | 刚收到消息不等于刚采集观测 |
| 采集时间 | 按证据时间计算，而非硬写 `FRESH` | 一个标签不能证明相机时钟正确或数据未被缓存 |
| 规划基础 | `Plan(request, intent, world)` 按调用传入世界，修订对着当前状态规划 | 合法计划不一定可在当前物理条件下执行 |
| 结果未知 | `UnknownOutcome` 阻止盲目重放 | 不是所有物理动作都不幂等，也不是文件操作都可撤销 |
| 身份与名称 | 房间名归一化、位姿委托和物体关联分别负责不同含义 | 名字相同或位置接近不等于同一实体 |

### `ENOENT` 与 `objects=0`：结论取决于接口契约

`ENOENT` 表示该路径解析在当次调用的命名空间中缺少某个组成部分，不能无条件外推到其他挂载、时间或业务实体。`objects=0` 通常仅表示当前观测没有输出检测，不能直接推出目标不存在。

项目的三条历史 grounding 根因就有相同表象：在错误房间查找、物体数据被哈希拒识、绑定表只覆盖 `cup/red`。它们分别需要修正规划、数据验证和绑定覆盖，而非都让机器人再找一次。

### 计划携带状态，名称统一解释

`Planner.Plan` 传入 `world`，修订重新读取状态，是防止陈旧计划的重要设计。软件 Agent 对并发文件、数据库版本和部署状态同样需要快照或版本前置条件。

4.3的三张别名表则说明：历史分层对同一房间名可能给出不同解释。统一归一化是修复路径，但还需保留位姿版本与授权；不能只改字符串就宣布空间身份正确。
