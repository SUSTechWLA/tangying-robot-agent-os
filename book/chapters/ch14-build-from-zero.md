...
| 4 | **时间比较用整毫秒截断** | runtime 的采集时间分辨率就是整毫秒，**亚毫秒顺序原理上不可观测**——这不是容差，是承认极限 |
...
**在通用 coding agent 里这个取舍是反过来的**——测试红了，再跑一次没有代价。
...
⑦ 执行入口
      ...
      └─ decision = safety.start(command)   ← ★ 唯一否决点
      ▼
...
理由：如果一个命令是**重复投递**的，它**不应该再走一次安全评估**——因为第一次已经评估过了，结果是"允许"。**重新评估可能因为环境变化给出不同答案，从而产生"同一个逻辑调用得到两种结果"。**

**细节 3：第 ⑧ 步是唯一能否决的层。**

**"唯一否决点"是一个重要的架构属性**：如果安全否决分散在五处，你就无法回答"这个命令为什么被允许"。
...
| **4** | 单通道 + 唯一否决点 | 加 `origin` 字段 → 测试红；`READY` 无 `EXECUTING` 边 |
...
| 5 | 用时间戳的浮点值直接比较 | **整毫秒截断**——分辨率是物理限制，不是容差 |
